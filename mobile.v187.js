...
function importPcProgrammingV165(){
  ...
  const marker=`forgelabMobileProgrammingImport:v187:${currentUser.id}`;
...
const jDraftSave=x=>localStorage.setItem(jDraftKey(),JSON.stringify(x));
...
function drawJournal(){
 ...
 document.querySelector('#jNew').onclick=()=>{journalDraft={id:jId(),sessionName:'',date:jToday(),duration:0,notes:'',exercises:[]};drawJournalEdit(false,isDraft)};
...
function drawJournalEdit(readOnly=false,isDraft=false){
 ...
 v.innerHTML=`<div class="j-edit-head"><button id="jBack">‹ Journal</button><b>${readOnly?'Séance enregistrée':isDraft?'Brouillon à compléter':'Saisie de séance'}</b></div>
...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v187.png" alt="" aria-hidden="true">
      <div class="auth-brand">FORGELAB</div>
      <div class="auth-sub">ATLAS · ESPACE PERSONNEL</div>
      <div class="auth-version"><span>v1.8.7</span></div>
    </div>

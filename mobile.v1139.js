...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1139.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 let ref=null,laurel=null,footerScene=null;try{ref=await load('template-reference-v1139.png')}catch(_){ }try{laurel=await load('auth-laurel-realistic-v1139.png')}catch(_){ }try{footerScene=await load('footer-mountains-man-v1139.png')}catch(_){ }
 if(ref)g.drawImage(ref,0,0,W,H);else{g.fillStyle='#03080c';g.fillRect(0,0,W,H)}
 const white='#f5efe6',muted='#b9aa98',blue='#c87532',green='#e0a34a',grey='#9d9185',dark='rgba(11,7,5,.975)',panel='rgba(20,12,8,.975)';
 const ember='#b94a22',copper='#c87532',gold='#e0a34a',iron='#241914';
 ...
 // Header v1.13.9 : identité Forge — métal sombre, cuivre, braise et or chaud.
 const hg=g.createLinearGradient(0,0,W,0);hg.addColorStop(0,'#080504');hg.addColorStop(.5,'#21120b');hg.addColorStop(1,'#080504');g.fillStyle=hg;g.fillRect(0,0,W,112);
 const glow=g.createRadialGradient(512,52,0,512,52,330);glow.addColorStop(0,'rgba(210,91,35,.18)');glow.addColorStop(1,'rgba(210,91,35,0)');g.fillStyle=glow;g.fillRect(0,0,W,112);
 line(24,103,1000,103,'#5d321f',1);line(312,104,712,104,copper,2);
 if(laurel)g.drawImage(laurel,456,9,112,88);
 // Titre : cartouche forgé plus travaillé, sans slogan parasite.
 const tg=g.createLinearGradient(0,116,0,212);tg.addColorStop(0,'rgba(31,18,12,.99)');tg.addColorStop(1,'rgba(8,6,5,.99)');g.fillStyle=tg;g.fillRect(52,118,920,88);
 g.strokeStyle='#5f3824';g.lineWidth=1;g.strokeRect(52.5,118.5,919,87);
 g.strokeStyle=copper;g.lineWidth=2;g.beginPath();g.moveTo(94,130);g.lineTo(210,130);g.moveTo(814,130);g.lineTo(930,130);g.stroke();
 fit((meta.name||'PROGRAMME FORGELAB').toUpperCase(),512,157,800,43,white,900,'center');
 txt('FORGELAB · PROGRAMMATION',512,190,11,gold,700,'center');
 // Métadonnées : quatre plaques métalliques avec accents cuivre/or.
 box(24,221,976,82,'rgba(10,7,5,.99)');const info=[['DURÉE',`${meta.duration} SEMAINES`],['OBJECTIF','HYPERTROPHIE'],['FRÉQUENCE',`${activeDays.length} SÉANCE${activeDays.length>1?'S':''} / SEMAINE`],['TRAME',`${meta.rhythm} + DELOAD`]];info.forEach((a,i)=>{const x=26+i*244;const pg=g.createLinearGradient(x,224,x+232,296);pg.addColorStop(0,'rgba(43,26,17,.98)');pg.addColorStop(1,'rgba(15,10,7,.98)');g.fillStyle=pg;g.fillRect(x,224,232,75);g.strokeStyle=i%2?gold:copper;g.lineWidth=1.2;g.strokeRect(x+.5,224.5,231,74);g.fillStyle=i%2?gold:copper;g.fillRect(x,224,4,75);txt(a[0],x+20,247,11,muted,700);fit(a[1],x+20,276,198,16,white,850)});
 // Cycles / deloads.
 box(20,314,984,186,'rgba(11,7,5,.985)');const gap=12,pw=(976-gap*(ph.length-1))/ph.length;ph.forEach((q,i)=>{const x=24+i*(pw+gap),ac=q.deload?grey:blue;g.fillStyle=panel;g.fillRect(x,322,pw,172);g.strokeStyle=ac;g.strokeRect(x,322,pw,172);g.fillStyle=ac;g.fillRect(x,322,pw,37);txt(q.deload?`DELOAD ${ph.slice(0,i+1).filter(z=>z.deload).length}`:`CYCLE ${ph.slice(0,i+1).filter(z=>!z.deload).length}`,x+pw/2,341,14,q.deload?'#11171b':'#031016',900,'center');txt(`S${q.from}${q.to!==q.from?' – S'+q.to:''}`,x+pw/2,385,18,white,900,'center');txt(q.deload?'RÉCUPÉRATION':(i===0?'DÉVELOPPEMENT':i===ph.length-1?'SPÉCIALISATION':'INTENSIFICATION'),x+pw/2,425,13,q.deload?grey:blue,800,'center');fit(q.deload?'Volume réduit · récupération':'Progression · technique · qualité',x+pw/2,463,pw-18,11,muted,500,'center')});
 ...
 // Footer v1.13.9 : une seule scène continue, sans raccord ni inscription.
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1139.png" alt=""><strong>ForgeLab</strong></div>

...
function selectedPhaseKey(mode=programmingMode()){return `forgelabMobileProgrammingSelectedPhase:${currentUser?.id||'guest'}:${mode}`}
...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1174.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 let ref=null,laurel=null,footerScene=null,posterRef=null;try{ref=await load('template-reference-v1174.png')}catch(_){ }try{laurel=await load('auth-laurel-realistic-v1174.png')}catch(_){ }try{footerScene=await load('footer-mountains-man-v1174.png')}catch(_){ }try{posterRef=await load('poster-reference-v1174.png')}catch(_){ }
 const atlasFiles={pectoraux:'atlas-pectoraux-v1174.png',dos:'atlas-dos-v1174.png',deltoide_anterieur:'atlas-deltoide_anterieur-v1174.png',deltoide_lateral:'atlas-deltoide_lateral-v1174.png',deltoide_posterieur:'atlas-deltoide_posterieur-v1174.png',trapezes_superieurs:'atlas-trapezes_superieurs-v1174.png',trapezes_moyens:'atlas-trapezes_moyens-v1174.png',cou:'atlas-cou-v1174.png',biceps:'atlas-biceps-v1174.png',triceps:'atlas-triceps-v1174.png',avant_bras:'atlas-avant_bras-v1174.png',abdominaux:'atlas-abdominaux-v1174.png',lombaires:'atlas-lombaires-v1174.png',fessiers:'atlas-fessiers-v1174.png',adducteurs:'atlas-adducteurs-v1174.png',quadriceps:'atlas-quadriceps-v1174.png',ischios:'atlas-ischios-v1174.png',mollets:'atlas-mollets-v1174.png'};
 ...
 ph.forEach((q,i)=>{const x=24+i*(pw+gap),cx=x+pw/2,ac=q.deload?grey:gold;line(x+12,330,cx-58,330,q.deload?'rgba(169,160,146,.32)':'rgba(214,173,96,.34)',1);line(cx+58,330,x+pw-12,330,q.deload?'rgba(169,160,146,.32)':'rgba(214,173,96,.34)',1);txt(q.deload?`DELOAD ${ph.slice(0,i+1).filter(z=>z.deload).length}`:`CYCLE ${ph.slice(0,i+1).filter(z=>!z.deload).length}`,cx,330,11,ac,850,'center','Trebuchet MS');txt(`S${q.from}${q.to!==q.from?' – S'+q.to:''}`,cx,373,21,white,900,'center');txt(q.deload?'RÉCUPÉRATION':(i===0?'DÉVELOPPEMENT':i===ph.length-1?'SPÉCIALISATION':'INTENSIFICATION'),cx,414,11,ac,800,'center','Trebuchet MS');line(cx-28,435,cx+28,435,q.deload?'rgba(169,160,146,.35)':'rgba(214,173,96,.40)',1);fit(q.deload?'Volume réduit · récupération':'Progression · technique · qualité',cx,462,pw-20,10,muted,500,'center')});
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1174.png" alt=""><strong>ForgeLab</strong></div>
...
function fullProgramKey(){return `forgelabFullProgram:${currentUser?.id||'guest'}`}
...
function blockPrefix(id=selectedFullBlock().id){return `forgelabProgramBlock:${currentUser?.id||'guest'}:${id}`}
...
function addFullBlock(){
 ...
 const name=(prompt('Nom du nouveau cycle :',`Cycle ${n}`)||'').trim();if(!name)return;
...
function renameFullBlock(){const x=fullProgram(),b=x.blocks.find(z=>z.id===x.selected);if(!b)return;const n=(prompt('Nom du cycle :',b.name)||'').trim();if(!n)return;b.name=n;saveFullProgram(x);const m=programmingMeta();m.name=n;saveProgrammingMeta(m);saveCurrentIntoBlock(b.id);drawProgramming()}
function deleteFullBlock(){const x=fullProgram();if(x.blocks.length<=1)return alert('Le programme doit contenir au moins un cycle.');const b=x.blocks.find(z=>z.id===x.selected);if(!confirm(`Supprimer ${b?.name||'ce cycle'} ?`))return;x.blocks=x.blocks.filter(z=>z.id!==x.selected);x.selected=x.blocks[0].id;saveFullProgram(x);loadBlockIntoCurrent(x.selected);drawProgramming()}
...
async function generateCompleteProgram(){
 ...
 fit((fp.name||'PROGRAMME FORGELAB').toUpperCase(),W/2,72,W-180,44,gold,900,'center');txt(`PROGRAMME COMPLET · ${blocks.length} CYCLE${blocks.length>1?'S':''}`,W/2,128,18,muted,700,'center');g.strokeStyle='rgba(226,189,112,.55)';g.beginPath();g.moveTo(120,166);g.lineTo(W-120,166);g.stroke();
 blocks.forEach(({b,i,meta,phases})=>{const y=headerH+i*(blockH+gap);g.fillStyle='rgba(15,14,10,.98)';g.fillRect(55,y,W-110,blockH);g.strokeStyle='rgba(226,189,112,.55)';g.lineWidth=2;g.strokeRect(55,y,W-110,blockH);txt(`CYCLE ${i+1}`,88,y+42,15,gold,900);fit((b.name||meta.name||`Cycle ${i+1}`).toUpperCase(),88,y+82,W-240,30,white,900);txt(`${meta.duration} semaines · trame ${meta.rhythm} + deload`,88,y+118,14,muted,600);
...
function drawProgrammingChooser(){
 ...
 const hasDraft=hasProgrammingDraft(),active=programmingMeta('active'),draft=hasDraft?programmingMeta('draft'):null,fp=fullProgram();
 v.innerHTML=title('Programmation')+`<section class="program-choice-screen">
   <div class="program-choice-intro"><small>ESPACE PROGRAMMATION</small><strong>Quel programme veux-tu ouvrir ?</strong><span>Choisis un espace pour afficher sa programmation.</span></div>
   <button type="button" class="program-choice-card active" data-program-choice="active">
     <span class="program-choice-icon">A</span><div><small>PROGRAMME ACTIF</small><strong>${esc(state.blockName||active.name||'Bloc actif')}</strong><span>${state.blockDuration||active.duration} semaine${Number(state.blockDuration||active.duration)>1?'s':''} · ${nutritionLabel()}</span></div><b>›</b>
   </button>
   <button type="button" class="program-choice-card draft ${hasDraft?'':'empty'}" data-program-choice="draft">
     <span class="program-choice-icon">B</span><div><small>BROUILLON</small><strong>${hasDraft?esc(draft.name||'Programme en préparation'):'Nouveau programme'}</strong><span>${hasDraft?`${draft.duration} semaine${draft.duration>1?'s':''} · modifiable sans toucher à l’actif`:'Créer ton prochain programme'}</span></div><b>${hasDraft?'›':'＋'}</b>
   </button>
   <div class="program-choice-foot"><span>${esc(fp.name||'Programme ForgeLab')}</span><i>DISCIPLINA · CONSTANTIA · PROGRESSUS</i></div>
 </section>`;
 v.querySelector('[data-program-choice="active"]').onclick=()=>{programmingScreen='editor';setProgrammingMode('active')};
 v.querySelector('[data-program-choice="draft"]').onclick=()=>{programmingScreen='editor';if(hasDraft)setProgrammingMode('draft');else createProgrammingDraft(true)};
}
drawProgramming=function(){
 if(programmingScreen==='chooser'){drawProgrammingChooser();return}
 _drawProgrammingV1162();const v=document.querySelector('#view');if(!v)return;const fp=fullProgram(),sel=selectedFullBlock();
 const back=document.createElement('button');back.type='button';back.className='program-back-choice';back.innerHTML='‹ <span>Programmation</span>';back.onclick=()=>{programmingScreen='chooser';drawProgramming()};
 const heading=v.querySelector('.section-title');if(heading)heading.insertAdjacentElement('afterend',back);else v.prepend(back);
 const host=document.createElement('section');host.className='full-program-manager';host.innerHTML=`<div class="full-program-head"><div><small>PROGRAMME COMPLET</small><strong>${esc(fp.name||'Programme ForgeLab')}</strong><input class="full-program-name" data-full-program-name value="${esc(fp.name||'Programme ForgeLab')}" aria-label="Nom du programme complet"></div><button data-full-add>+ Cycle</button></div><div class="full-block-tabs">${fp.blocks.map((b,i)=>`<button class="${b.id===fp.selected?'active':''}" data-full-block="${esc(b.id)}"><small>CYCLE ${i+1}</small><b>${esc(b.name)}</b></button>`).join('')}</div><div class="full-program-actions"><button class="secondary" data-full-rename>Renommer le cycle</button><button class="secondary" data-full-delete>Supprimer</button><button data-full-current>Générer ce cycle</button><button class="gold" data-full-complete>Générer le programme complet</button></div>`;
 const anchor=v.querySelector('.program-mobile-steps')||v.firstChild;v.insertBefore(host,anchor);
 host.querySelector('[data-full-program-name]').addEventListener('change',e=>{const x=fullProgram();x.name=(e.target.value||'').trim()||'Programme ForgeLab';saveFullProgram(x);drawProgramming()});host.querySelector('[data-full-add]').onclick=addFullBlock;host.querySelector('[data-full-rename]').onclick=renameFullBlock;host.querySelector('[data-full-delete]').onclick=deleteFullBlock;host.querySelector('[data-full-current]').onclick=()=>{saveCurrentIntoBlock();generateProgramImage()};host.querySelector('[data-full-complete]').onclick=generateCompleteProgram;host.querySelectorAll('[data-full-block]').forEach(x=>x.onclick=()=>selectFullBlock(x.dataset.fullBlock));
};

...
function importPcProgrammingV165(){
  ...
  const marker=`forgelabMobileProgrammingImport:v1116:${currentUser.id}`;
...
async function repairSeriesFromCarnet(cloudRows){
  // v1.11.6 : fonction conservée uniquement pour compatibilité interne.
...
function jSeriesBaselineForDraft(id){
  ...
  // Compatibilité immédiate avec les séances déjà validées avant v1.11.6 :
...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1116.png" alt="" aria-hidden="true">
      <div class="auth-brand">FORGELAB</div>
      <div class="auth-sub">ATLAS · ESPACE PERSONNEL</div>
      <div class="auth-version"><span>v1.11.6</span></div>
    </div>
...
function trackingMuscuTrend(done,minimum,inTarget,under,over,totalGroups){
  ...
  if(under===0&&over===0)return `Semaine complète : ${pct}% du volume minimum réalisé et ${inTarget}/${totalGroups} groupes musculaires dans leur cible.`;
  if(under===0)return `Volume minimum atteint à ${pct}%. ${inTarget}/${totalGroups} groupes sont dans leur cible${over?`, avec ${over} au-dessus du maximum prévu`:''}.`;
...
// v1.11.6 : aucun nettoyage automatique de Séries.

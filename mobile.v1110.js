...
function importPcProgrammingV165(){
  ...
  const marker=`forgelabMobileProgrammingImport:v1110:${currentUser.id}`;
...
function jSeriesBaselineForDraft(id){
  ...
  // Compatibilité immédiate avec les séances déjà validées avant v1.11.0 :
...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1110.png" alt="" aria-hidden="true">
      <div class="auth-brand">FORGELAB</div>
      <div class="auth-sub">ATLAS · ESPACE PERSONNEL</div>
      <div class="auth-version"><span>v1.11.0</span></div>
    </div>
  ...
  return `<article class="card ${d.priority.toLowerCase()} cardio-series-card">
    ...
    <section class="base-run-entry">
      ...
      <div class="base-run-fields">
        ...
        <label>Allure moy.<input id="baseRunPace" type="text" inputmode="numeric" placeholder="7:13"><i>/km</i></label>
...
function wireCardioSeries(v){
  ...
  if(save)save.onclick=()=>{
    ...
    const pace=(v.querySelector('#baseRunPace')?.value||'').trim();
    const avgHr=+v.querySelector('#baseRunHr')?.value||0;
    const rpe=+v.querySelector('#baseRunRpe')?.value||0;
    if(!date||duration<=0||!paceToSeconds(pace)||avgHr<=0||rpe<1||rpe>10){alert('Renseigne : date, durée, allure au format 7:13, FC moyenne et RPE /10.');return}

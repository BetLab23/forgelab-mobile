...
function importPcProgrammingV165(){
  ...
  const marker=`forgelabMobileProgrammingImport:v199:${currentUser.id}`;
...
function drawJournalEdit(readOnly=false,isDraft=false){
 ...
 v.querySelectorAll('.j-ex').forEach(card=>{const i=+card.dataset.i,e=d.exercises[i];
  ...
  const rmEx=card.querySelector('[data-rmex]');if(rmEx)rmEx.onclick=()=>{d.exercises.splice(i,1);persistDraftNow();drawJournalEdit(false,isDraft)};
...
function jExHtml(e,i,ro){
 return `<section class="j-card j-ex" data-i="${i}">
   ...
   <div class="j-exhead"><label>Groupe<select data-muscle ${ro?'disabled':''}>${jMuscles(e.muscle)}</select></label><label>Exercice<select data-name ${ro?'disabled':''}>${jExercises(e.muscle,e.name)}</select></label>${ro||e.muscleKey?'':`<button data-rmex>×</button>`}</div>
...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v199.png" alt="" aria-hidden="true">
      <div class="auth-brand">FORGELAB</div>
      <div class="auth-sub">ATLAS · ESPACE PERSONNEL</div>
      <div class="auth-version"><span>v1.9.9</span></div>
    </div>

...
function importPcProgrammingV165(){
  ...
  const marker=`forgelabMobileProgrammingImport:v1104:${currentUser.id}`;
...
async function repairSeriesFromCarnet(cloudRows){
  const rows=Array.isArray(cloudRows)?cloudRows.map(x=>({...x})):[];
  const required=jSeriesRequirementsFromHistory();
  if(!required.size)return rows;

  const byKey=new Map(rows.map((r,i)=>[`${+r.week}|${r.muscle_key}|${+r.day}`,{r,i}]));
  const fixes=[];

  for(const [k,minValue] of required.entries()){
    const [week,muscleKey,day]=k.split('|');
    const current=byKey.get(k)?.r;
    const currentValue=Math.max(0,+current?.series||0);

    // On ne réduit jamais une saisie existante. On ne fait que restaurer
    // les séries validées dans le Carnet qui ont disparu de Supabase.
    ...
    if(current){
      ...
      rows.push(row);
      byKey.set(k,{r:row,i:rows.length-1});
    }
  }
  ...
  if(fixes.length){
    ...
    console.info(`ForgeLab : ${fixes.length} donnée(s) Séries restaurée(s) depuis le Carnet.`);
  }
  return rows;
}
...
function jSeriesBaselineForDraft(id){
  ...
  // Compatibilité immédiate avec les séances déjà validées avant v1.10.4 :
...
function drawJournalEdit(readOnly=false,isDraft=false){
 ...
 document.querySelector('#jValidate').onclick=async()=>{d.sessionName=(document.querySelector('#jSessionName').value||'').trim();d.date=document.querySelector('#jDate').value||jToday();d.duration=Math.max(0,+document.querySelector('#jDur').value||0);d.notes=document.querySelector('#jNotes').value||'';d.updatedAt=new Date().toISOString();d.exercises=d.exercises.filter(e=>e.name&&e.sets.some(s=>(+s.kg||0)||(+s.reps||0)));if(!d.exercises.length){alert('Renseigne au moins une série.');return}const a=jLoad();
   ...
   // Une séance validée doit toujours être reflétée dans Séries et donc dans Suivi.
   try{state.series=await repairSeriesFromCarnet(state.series)}catch(e){console.error('Réparation Séries après validation',e)}
   journalDraft=null;drawJournal()};
}
...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1104.png" alt="" aria-hidden="true">
      <div class="auth-brand">FORGELAB</div>
      <div class="auth-sub">ATLAS · ESPACE PERSONNEL</div>
      <div class="auth-version"><span>v1.10.4</span></div>
    </div>
...
async function load(render=true){try{
  ...
  if(!editingNow){
    c=await repairSeriesFromCarnet(c);
    state.series=c;
  }
...
function draw(){
  ensureAccountButton();
  document.querySelector('#weekTitle').innerHTML=`<span class="week-kicker">S${state.week}</span><span class="week-block-name">${esc(state.blockName||'Bloc actif')}</span>`;
...
function cardioTrackingHtml(){
  ...
  return `<section class="card cardio-tracking-card ${d.priority.toLowerCase()}">
    <div class="cardio-track-head">
      <div><small>CONDITIONNEMENT</small><div class="muscle">Cardio</div><div class="range">Priorité ${d.priority} · ${d.modalities.length} modalité${d.modalities.length>1?'s':''}</div></div>
    ...
    <div class="cardio-track-modalities">${d.modalities.map(m=>{
      const mdone=Object.values(week[m.id]||{}).reduce((a,n)=>a+(+n||0),0),mp=m.target?Math.min(100,Math.round(mdone/m.target*100)):100;
      return `<div class="cardio-track-modality"><div><b>${esc(m.type)}</b><span>${mdone} / ${m.target}</span></div><i><em style="width:${mp}%"></em></i></div>`;
    }).join('')}</div>
...
function drawTracking(){
  ...
  const atMin=active.filter(p=>total(p.muscle_key)>=(+p.target_min||0)).length;
  const cardioOn=cardioData().enabled,cd=cardioOn?cardioTotalDone():0,ct=cardioOn?cardioTotalTarget():0;
  let v=document.querySelector('#view');
  v.innerHTML=title('Suivi')+
  `<section class="tracking-hero">
    <div class="tracking-hero-copy"><small>FORGELAB · S${state.week}</small><h3>${esc(state.blockName||'Bloc actif')}</h3><p>Vue d’ensemble de ta charge hebdomadaire.</p></div>
    <div class="overall-ring"><b>${pct}%</b><span>musculation</span></div>
  </section>`+
  `<div class="tracking-kpi-grid">
    <div><small>SÉRIES</small><b>${done}</b><span>réalisées</span></div>
    <div><small>MINIMUM</small><b>${minimum}</b><span>cumulé</span></div>
    <div><small>RESTANT</small><b>${remain}</b><span>jusqu’aux cibles</span></div>
    <div class="${cardioOn?'cardio-kpi':''}"><small>CARDIO</small><b>${cardioOn?`${cd}/${ct}`:'—'}</b><span>${cardioOn?'séances':'non activé'}</span></div>
  </div>`+
  weeklyTodoRecap()+
  cardioTrackingHtml()+
  `<div class="tracking-section-label"><span>DÉTAIL MUSCULAIRE</span><b>${atMin}/${active.length} au minimum</b></div>`+
  active.map(p=>{

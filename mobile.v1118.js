...
function importPcProgrammingV165(){
  ...
  const marker=`forgelabMobileProgrammingImport:v1118:${currentUser.id}`;
...
async function repairSeriesFromCarnet(cloudRows){
  // v1.11.8 : fonction conservée uniquement pour compatibilité interne.
...
function jSeriesBaselineForDraft(id){
  ...
  // Compatibilité immédiate avec les séances déjà validées avant v1.11.8 :
...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1118.png" alt="" aria-hidden="true">
      <div class="auth-brand">FORGELAB</div>
      <div class="auth-sub">ATLAS · ESPACE PERSONNEL</div>
      <div class="auth-version"><span>v1.11.8</span></div>
    </div>
...
function trackingSeriesSessionCount(){
  // Source de vérité : onglet Séries.
  // Le modèle actuel stocke les séries par groupe + jour : un jour avec au moins
  // une série réalisée = une séance détectée. Aucun Carnet n'est nécessaire.
  // On ne déduit jamais artificiellement une 2e séance à partir du nombre de groupes.
  let count=0;
  for(let d=1;d<=7;d++){
    const hasSeries=activePriorities().some(p=>(+getSeries(p.muscle_key,d)||0)>0);
    if(hasSeries) count++;
  }
  return count;
}
function trackingMuscuTrend(done,minimum,inTarget,under,over,totalGroups){
  ...
  if(under===0)return `Semaine complète : ${inTarget}/${totalGroups} objectifs musculaires atteints. ${over} groupe${over>1?'s':''} au-dessus du volume maximum prévu.`;
  if(pct>=90)return `Semaine presque complète : ${pct}% du volume minimum réalisé. Il reste ${under} groupe${under>1?'s':''} musculaire${under>1?'s':''} à compléter.`;
  if(pct>=70)return `Bonne progression : ${pct}% du volume minimum réalisé. ${under} groupe${under>1?'s':''} reste${under>1?'nt':''} à compléter.`;
  return `Semaine en cours : ${pct}% du volume minimum réalisé. Priorité aux ${under} groupe${under>1?'s':''} encore sous leur cible.`;
}
function trackingMuscuSummary(){
  ...
  const trend=trackingMuscuTrend(done,minimum,inTarget,under,over,active.length);
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    ...
    <div class="story-status-grid">
      <div class="ok"><b>${inTarget}</b><strong>GROUPES MUSCULAIRES</strong><span>objectif atteint</span></div>
      <div class="low"><b>${under}</b><strong>GROUPES MUSCULAIRES</strong><span>à compléter</span></div>
      <div class="high"><b>${over}</b><strong>GROUPES MUSCULAIRES</strong><span>volume dépassé</span></div>
    </div>
...
// v1.11.8 : aucun nettoyage automatique de Séries.

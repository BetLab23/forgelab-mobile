...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1123.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 const W=1600, margin=40, cols=Math.min(5,Math.max(1,activeDays.length)), rows=Math.ceil(activeDays.length/5);
 const dayH=620, H=720 + 235 + rows*(dayH+24) + 380;
 const c=document.createElement('canvas');c.width=W;c.height=H;const g=c.getContext('2d');
 const bg='#05090c', panel='#0a1116', panel2='#0d151b', blue='#16a6f3', green='#08c39a', grey='#aeb4b8', white='#f3f5f6', muted='#9ba7af', lineC='#32424d', gold='#d7b46a';
 const rr=(x,y,w,h,r,fill,stroke,lw=2)=>{g.beginPath();g.roundRect(x,y,w,h,r);if(fill){g.fillStyle=fill;g.fill()}if(stroke){g.strokeStyle=stroke;g.lineWidth=lw;g.stroke()}};
 const txt=(t,x,y,size,color=white,weight=600,align='left',family='Arial')=>{g.fillStyle=color;g.font=`${weight} ${size}px ${family}`;g.textAlign=align;g.textBaseline='middle';g.fillText(String(t),x,y)};
 const fit=(t,x,y,max,size,color=white,weight=800,align='left')=>{let z=size;while(z>20){g.font=`${weight} ${z}px Arial`;if(g.measureText(String(t)).width<=max)break;z-=2}txt(t,x,y,z,color,weight,align)};
 const line=(x1,y1,x2,y2,color=lineC,w=2)=>{g.beginPath();g.moveTo(x1,y1);g.lineTo(x2,y2);g.strokeStyle=color;g.lineWidth=w;g.stroke()};
 ...
 const anatomy=(cx,cy,names,accent,scale=1)=>{const Z=new Set(names.map(zone));g.save();g.translate(cx,cy);g.scale(scale,scale);g.lineJoin='round';g.lineCap='round';
   const base='#26343c', edge='#52616a';g.fillStyle=base;g.strokeStyle=edge;g.lineWidth=2;
   g.beginPath();g.ellipse(0,-72,18,22,0,0,Math.PI*2);g.fill();g.stroke();
   g.beginPath();g.moveTo(-11,-50);g.lineTo(-36,-30);g.lineTo(-31,22);g.lineTo(-17,56);g.lineTo(0,66);g.lineTo(17,56);g.lineTo(31,22);g.lineTo(36,-30);g.lineTo(11,-50);g.closePath();g.fill();g.stroke();
   g.beginPath();g.moveTo(-34,-28);g.lineTo(-54,-5);g.lineTo(-61,48);g.lineTo(-49,51);g.lineTo(-35,8);g.closePath();g.fill();g.stroke();
   g.beginPath();g.moveTo(34,-28);g.lineTo(54,-5);g.lineTo(61,48);g.lineTo(49,51);g.lineTo(35,8);g.closePath();g.fill();g.stroke();
   g.beginPath();g.moveTo(-16,57);g.lineTo(-31,92);g.lineTo(-27,151);g.lineTo(-10,151);g.lineTo(-2,75);g.closePath();g.fill();g.stroke();
   g.beginPath();g.moveTo(16,57);g.lineTo(31,92);g.lineTo(27,151);g.lineTo(10,151);g.lineTo(2,75);g.closePath();g.fill();g.stroke();
   g.fillStyle=accent;g.globalAlpha=.95;
   if(Z.has('neck'))g.fillRect(-9,-52,18,16);
   if(Z.has('shoulder')){g.beginPath();g.ellipse(-28,-27,16,13,-.2,0,Math.PI*2);g.ellipse(28,-27,16,13,.2,0,Math.PI*2);g.fill()}
   if(Z.has('chest')){g.beginPath();g.ellipse(-13,-15,17,14,0,0,Math.PI*2);g.ellipse(13,-15,17,14,0,0,Math.PI*2);g.fill()}
   if(Z.has('back')){g.beginPath();g.moveTo(-28,-20);g.lineTo(-17,28);g.lineTo(0,46);g.lineTo(17,28);g.lineTo(28,-20);g.lineTo(0,-5);g.closePath();g.fill()}
   if(Z.has('arms')){g.beginPath();g.ellipse(-46,8,10,30,-.18,0,Math.PI*2);g.ellipse(46,8,10,30,.18,0,Math.PI*2);g.fill()}
   if(Z.has('core')){g.beginPath();g.roundRect(-15,8,30,45,8);g.fill()}
   if(Z.has('legs')){g.beginPath();g.moveTo(-15,59);g.lineTo(-29,95);g.lineTo(-25,139);g.lineTo(-10,139);g.lineTo(-2,73);g.closePath();g.moveTo(15,59);g.lineTo(29,95);g.lineTo(25,139);g.lineTo(10,139);g.lineTo(2,73);g.closePath();g.fill()}
   g.restore();
 };
 // Background inspired by the approved reference: deep navy/black, thin blue structure, subtle light.
 g.fillStyle=bg;g.fillRect(0,0,W,H);let grd=g.createLinearGradient(0,0,W,0);grd.addColorStop(0,'rgba(0,0,0,.55)');grd.addColorStop(.52,'rgba(8,21,30,.72)');grd.addColorStop(1,'rgba(0,0,0,.65)');g.fillStyle=grd;g.fillRect(0,0,W,H);
 // Header with ForgeLab laurel identity.
 let logo=null;for(const src of ['auth-laurel-realistic-v1129.png','auth-laurel-realistic-v1127.png']){try{logo=await new Promise((res,rej)=>{const im=new Image();im.onload=()=>res(im);im.onerror=rej;im.src=src});if(logo)break}catch(_){}}
 if(logo)g.drawImage(logo,54,30,155,155);txt('FORGELAB',225,82,55,white,900);txt('TRAIN  TRACK  PROGRESS',228,137,18,white,500);txt('DISCIPLINE',W-205,50,15,muted,500);txt("AUJOURD’HUI",W-205,79,15,muted,500);txt('RÉSULTATS DEMAIN',W-205,108,15,muted,500);line(40,182,W-40,182,blue,3);
 fit((meta.name||'PROGRAMME FORGELAB').toUpperCase(),W/2,248,W-180,66,white,900,'center');txt('CONSTRUIRE AUJOURD’HUI LA MEILLEURE VERSION DE DEMAIN',W/2,305,20,white,500,'center');
 // Metadata cards
 const cards=[['▣','DURÉE',`${meta.duration} SEMAINES`],['▥','OBJECTIF','HYPERTROPHIE'],['▰','FRÉQUENCE',`${activeDays.length} SÉANCE${activeDays.length>1?'S':''} / SEMAINE`],['◷','TRAME',`${meta.rhythm} + DELOAD`]];const cw=(W-80-36)/4;cards.forEach((a,i)=>{const x=40+i*(cw+12);rr(x,350,cw,118,10,'rgba(8,15,20,.92)',lineC);txt(a[0],x+28,408,32,blue,800);txt(a[1],x+70,382,16,muted,600);fit(a[2],x+70,425,cw-90,24,white,800)});
 // Cycle ribbon
 let y=505;const pgap=12,pw=(W-80-pgap*(ph.length-1))/ph.length;ph.forEach((q,i)=>{const x=40+i*(pw+pgap),ac=q.deload?'#b5b9bc':blue;rr(x,y,pw,180,8,'rgba(8,16,22,.94)',ac);g.fillStyle=ac;g.fillRect(x,y,pw,45);txt(q.deload?'DELOAD '+Math.ceil((i+1)/2):'CYCLE '+(ph.slice(0,i+1).filter(z=>!z.deload).length),x+pw/2,y+23,19,q.deload?'#12171a':'#041017',900,'center');txt(`S${q.from}${q.to!==q.from?' – S'+q.to:''}`,x+pw/2,y+79,25,white,900,'center');txt(q.deload?'RÉCUPÉRATION':(i===0?'DÉVELOPPEMENT':i<ph.length-1?'INTENSIFICATION':'SPÉCIALISATION'),x+pw/2,y+122,18,q.deload?grey:blue,800,'center');txt(q.deload?'Volume réduit · récupération':'Progression · qualité',x+pw/2,y+155,15,muted,500,'center')});
 y=735;txt(`TRAME HEBDOMADAIRE (${activeDays.length} SÉANCE${activeDays.length>1?'S':''})`,40,y,31,white,900);txt('LES GROUPES ET SÉRIES SONT ISSUS DE TA PROGRAMMATION FORGELAB',W-40,y,14,muted,500,'right');y+=45;
 // Day cards: up to five per row, close to the approved reference.
 const cardGap=14, cardW=(W-80-cardGap*4)/5;
 activeDays.forEach((d,idx)=>{const row=Math.floor(idx/5),col=idx%5,x=40+col*(cardW+cardGap),yy=y+row*(dayH+24),entries=(p[d]||[]).filter(z=>+z.sets>0),accent=idx%2?green:blue,total=entries.reduce((a,z)=>a+(+z.sets||0),0);
   rr(x,yy,cardW,dayH,10,'rgba(7,13,18,.96)',accent);g.fillStyle=accent;g.globalAlpha=.22;g.fillRect(x,yy,cardW,54);g.globalAlpha=1;txt(d.toUpperCase(),x+cardW/2,yy+28,21,white,900,'center');
   const focus=entries.slice(0,3).map(z=>z.muscle_name).join(' · ');fit(focus||'SÉANCE',x+cardW/2,yy+78,cardW-28,18,white,800,'center');anatomy(x+cardW/2,yy+190,entries.map(z=>z.muscle_name),accent,.78);
   let ey=yy+330;entries.slice(0,7).forEach(z=>{fit(z.muscle_name,x+16,ey,cardW-105,17,white,600);txt(`${z.sets} séries`,x+cardW-16,ey,16,white,700,'right');ey+=39});
   rr(x+14,yy+dayH-78,cardW-28,58,8,'#111b22',null);txt('FOCUS',x+27,yy+dayH-58,15,accent,900);fit(focus||`${total} séries`,x+27,yy+dayH-35,cardW-52,13,white,500);
 });
 y += rows*(dayH+24)+18;
 // Lower professional information panels.
 const lowerGap=18, lowerW=(W-80-lowerGap*2)/3;const low=[['CARDIO','RUN BASSE INTENSITÉ · SEUIL · INTERVALLES','Selon le cycle et la récupération'],['PROGRESSION','CHARGES · RÉPÉTITIONS · EXÉCUTION','Progression mesurée semaine après semaine'],['NOTES','DELOAD · RÉCUPÉRATION · RESSENTI','La qualité d’exécution prime sur la charge']];low.forEach((a,i)=>{const x=40+i*(lowerW+lowerGap);rr(x,y,lowerW,170,10,'rgba(7,13,18,.96)',i===1?green:blue);txt(a[0],x+28,y+38,24,white,900);txt(a[1],x+28,y+87,16,i===1?green:blue,800);fit(a[2],x+28,y+126,lowerW-56,16,muted,500)});
 y+=205;
 // Cinematic footer, without childish illustrations.
 let fg=g.createLinearGradient(0,y,W,H);fg.addColorStop(0,'#111b23');fg.addColorStop(1,'#020405');g.fillStyle=fg;g.fillRect(0,y,W,H-y);g.globalAlpha=.28;for(let i=0;i<10;i++){g.beginPath();g.moveTo(i*190-80,H-85);g.lineTo(i*190+60,y+70+(i%3)*30);g.lineTo(i*190+210,H-85);g.closePath();g.fillStyle='#263844';g.fill()}g.globalAlpha=1;
 txt('PLUS QU’UN PROGRAMME',W*.34,y+82,27,white,900);txt('UN PROCESSUS.',W*.34,y+120,27,white,900);line(W*.34,y+151,W*.34+115,y+151,blue,6);txt('DAILY WINS.',W*.67,y+100,44,white,700,'center','Georgia');txt('« La régularité d’aujourd’hui crée la liberté de demain. »',W*.78,y+172,20,white,500,'center','Georgia');
 line(40,H-100,W-40,H-100,lineC,1);txt('FORGELAB',40,H-58,27,white,900);txt('DISCIPLINA · CONSTANTIA · PROGRESSUS',W-40,H-58,15,muted,500,'right');
 const blob=await new Promise(resolve=>c.toBlob(resolve,'image/png',1));if(!blob){alert("Impossible de générer l’image sur cet appareil.");return}
 const safe=String(meta.name||'programme').replace(/[^a-z0-9]+/gi,'_');const file=new File([blob],`ForgeLab_${safe}.png`,{type:'image/png'});
 try{if(navigator.share&&navigator.canShare&&navigator.canShare({files:[file]})){await navigator.share({files:[file],title:`ForgeLab · ${meta.name||'Programme'}`});return}}catch(err){if(err&&err.name==='AbortError')return}
 const url=URL.createObjectURL(blob),w=window.open(url,'_blank');if(w){setTimeout(()=>URL.revokeObjectURL(url),120000);return}const a=document.createElement('a');a.href=url;a.download=file.name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),30000);
}
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1123.png" alt=""><strong>ForgeLab</strong></div>

...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1169.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 let ref=null,laurel=null,footerScene=null,posterRef=null;try{ref=await load('template-reference-v1169.png')}catch(_){ }try{laurel=await load('auth-laurel-realistic-v1169.png')}catch(_){ }try{footerScene=await load('footer-mountains-man-v1169.png')}catch(_){ }try{posterRef=await load('poster-reference-v1169.png')}catch(_){ }
 const atlasFiles={pectoraux:'atlas-pectoraux-v1169.png',dos:'atlas-dos-v1169.png',deltoide_anterieur:'atlas-deltoide_anterieur-v1169.png',deltoide_lateral:'atlas-deltoide_lateral-v1169.png',deltoide_posterieur:'atlas-deltoide_posterieur-v1169.png',trapezes_superieurs:'atlas-trapezes_superieurs-v1169.png',trapezes_moyens:'atlas-trapezes_moyens-v1169.png',cou:'atlas-cou-v1169.png',biceps:'atlas-biceps-v1169.png',triceps:'atlas-triceps-v1169.png',avant_bras:'atlas-avant_bras-v1169.png',abdominaux:'atlas-abdominaux-v1169.png',lombaires:'atlas-lombaires-v1169.png',fessiers:'atlas-fessiers-v1169.png',adducteurs:'atlas-adducteurs-v1169.png',quadriceps:'atlas-quadriceps-v1169.png',ischios:'atlas-ischios-v1169.png',mollets:'atlas-mollets-v1169.png'};
 ...
 // Header v1.16.9 : zone entièrement reconstruite pour supprimer tout élément fantôme du template.
 ...
 // v1.16.9 — composition éditoriale ForgeLab : pas d'icônes UI, pas de cartes.
 ...
 const anatomy=(cx,cy,names,accent)=>{
   // v1.16.9: afficher TOUS les groupes travaillés, au lieu du seul premier groupe.
 ...
 // Footer v1.16.9 : scène continue + signature DAILY WINS. dans le ciel.
  ...
  const rows=DAYS.map(day=>{
    ...
    const items=entries.length?entries.map(x=>{
      ...
      return `<div class="mobile-program-entry" data-id="${esc(x.id)}" data-muscle-name="${esc(x.muscle_name)}">
        ...
        ${(x.exercises||[]).length?`<div class="program-exercise-visible" aria-label="Exercices programmés">${(x.exercises||[]).map(e=>`<div><strong>${esc(e.name)}</strong><span>${Number(e.sets||0)} série${Number(e.sets||0)>1?'s':''} · repos ${Number(e.rest||90)<60?Number(e.rest||90)+' s':(Number(e.rest||90)/60)+' min'}</span></div>`).join('')}</div>`:'<div class="program-exercise-visible empty">Aucun exercice choisi</div>'}
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1169.png" alt=""><strong>ForgeLab</strong></div>
...
/* ===== ForgeLab v1.16.9 — Programme complet / multi-blocs ===== */

...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1176.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 let ref=null,laurel=null,footerScene=null,posterRef=null;try{ref=await load('template-reference-v1176.png')}catch(_){ }try{laurel=await load('auth-laurel-realistic-v1176.png')}catch(_){ }try{footerScene=await load('footer-mountains-man-v1176.png')}catch(_){ }try{posterRef=await load('poster-reference-v1176.png')}catch(_){ }
 const atlasFiles={pectoraux:'atlas-pectoraux-v1176.png',dos:'atlas-dos-v1176.png',deltoide_anterieur:'atlas-deltoide_anterieur-v1176.png',deltoide_lateral:'atlas-deltoide_lateral-v1176.png',deltoide_posterieur:'atlas-deltoide_posterieur-v1176.png',trapezes_superieurs:'atlas-trapezes_superieurs-v1176.png',trapezes_moyens:'atlas-trapezes_moyens-v1176.png',cou:'atlas-cou-v1176.png',biceps:'atlas-biceps-v1176.png',triceps:'atlas-triceps-v1176.png',avant_bras:'atlas-avant_bras-v1176.png',abdominaux:'atlas-abdominaux-v1176.png',lombaires:'atlas-lombaires-v1176.png',fessiers:'atlas-fessiers-v1176.png',adducteurs:'atlas-adducteurs-v1176.png',quadriceps:'atlas-quadriceps-v1176.png',ischios:'atlas-ischios-v1176.png',mollets:'atlas-mollets-v1176.png'};
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1176.png" alt=""><strong>ForgeLab</strong></div>
...
function drawProgrammingChooser(){
 ...
 v.querySelector('[data-program-choice="active"]').onclick=()=>{setSelectedDraftId('');programmingScreen='editor';setProgrammingMode('active')};
 v.querySelectorAll('[data-program-draft-id]').forEach(b=>b.onclick=()=>{setSelectedDraftId(b.dataset.programDraftId);programmingScreen='editor';setProgrammingMode('draft')});
...
/* ===== ForgeLab v1.17.6 — Brouillon structuré : Programme > Trame > Blocs > Génération ===== */
let programBlockOpen=true;
...
function fl176EnsureStructure(){
  ...
  const wanted=fl176TrameCount(fp), old=Array.isArray(fp.blocks)?fp.blocks:[], next=[];
  for(let i=0;i<wanted;i++){
    let b=old.filter(x=>!x.deload)[i];
    if(!b)b={id:'bloc-'+Date.now().toString(36)+'-'+i,name:`Bloc ${roman(i+1)}`};
    b.deload=false;if(!b.name)b.name=`Bloc ${roman(i+1)}`;next.push(b);
  }
  let d=old.find(x=>x.deload||/^deload$/i.test(x.name||''));
  if(!d)d={id:'deload-'+Date.now().toString(36),name:'Deload',deload:true};
  d.deload=true;d.name='Deload';next.push(d);
  fp.blocks=next;if(!next.some(b=>b.id===fp.selected))fp.selected=next[0].id;saveFullProgram(fp);
  // Each top-level block owns one daily-programming surface; no nested cycle/deload hierarchy.
...
drawProgramming=function(){
  ...
  const dailyNodes=[legacySteps,v.querySelector('.program-mobile-actions'),v.querySelector('.program-mobile-library'),v.querySelector('.program-mobile-grid'),v.querySelector('.program-summary')].filter(Boolean);
  dailyNodes.forEach(n=>{if(!programBlockOpen)n.hidden=true;else n.hidden=false});
  ...
  config.querySelector('[data-fl176-duration]').onchange=e=>{fp=fullProgram();fp.totalDuration=Math.max(1,Math.min(52,Number(e.target.value)||14));saveFullProgram(fp);drawProgramming()};

...
async function generateProgramImage(){
 const meta=programmingMeta(),p=programmingData(),ph=phaseRanges(meta),W=1080,pad=70,line=42;
 const dayRows=DAYS.filter(d=>(p[d]||[]).some(x=>+x.sets>0));let H=360+ph.length*line+dayRows.reduce((a,d)=>a+70+(p[d]||[]).length*34,0);H=Math.max(900,H);
 const c=document.createElement('canvas');c.width=W;c.height=H;const x=c.getContext('2d');x.fillStyle='#100d09';x.fillRect(0,0,W,H);x.fillStyle='#d4a64d';x.font='700 28px Arial';x.fillText('FORGELAB · PROGRAMMATION',pad,70);x.fillStyle='#fff';x.font='700 48px Arial';x.fillText(meta.name||'Bloc ForgeLab',pad,130);x.fillStyle='#b9ad98';x.font='24px Arial';x.fillText(`${meta.duration} semaines · ${meta.rhythm} + deload`,pad,172);
 let y=235;x.fillStyle='#d4a64d';x.font='700 24px Arial';x.fillText('CYCLES',pad,y);y+=42;x.font='22px Arial';ph.forEach(q=>{x.fillStyle=q.deload?'#d9b36b':'#eee';x.fillText(`${q.label} · S${q.from}${q.to!==q.from?'–S'+q.to:''}${q.deload?' · DELOAD':''}`,pad,y);y+=line});y+=20;
 x.fillStyle='#d4a64d';x.font='700 24px Arial';x.fillText('SEMAINE TYPE',pad,y);y+=42;dayRows.forEach(d=>{x.fillStyle='#fff';x.font='700 25px Arial';const total=(p[d]||[]).reduce((a,z)=>a+(+z.sets||0),0);x.fillText(`${d} · ${total} séries`,pad,y);y+=36;x.font='21px Arial';(p[d]||[]).forEach(z=>{x.fillStyle='#c8c0b4';x.fillText(`${z.muscle_name} — ${z.sets}`,pad+28,y);y+=34});y+=20});
 ...
 try{
   if(navigator.share&&navigator.canShare&&navigator.canShare({files:[file]})){await navigator.share({files:[file],title:`ForgeLab · ${meta.name||'Programme'}`});return}
 }catch(err){if(err&&err.name==='AbortError')return}
 const url=URL.createObjectURL(blob);const w=window.open(url,'_blank');
 if(w){setTimeout(()=>URL.revokeObjectURL(url),120000);alert("L’image est ouverte. Sur iPhone : appui long sur l’image puis « Enregistrer dans Photos ».");return}

...
function importPcProgrammingV165(){
  ...
  const marker=`forgelabMobileProgrammingImport:v182:${currentUser.id}`;
...
const jSave=x=>localStorage.setItem(jKey(),JSON.stringify(x));
...
function jPrev(s){
 ...
 return jLoad().filter(x=>x.id!==s.id&&(x.exercises||[]).map(e=>e.name).filter(Boolean).sort().join('|')===names).sort((a,b)=>(b.date||'').localeCompare(a.date||''))[0]||null
}
function jMuscles(sel=''){return Object.keys(JOURNAL_LIBRARY).map(x=>`<option ${x===sel?'selected':''}>${esc(x)}</option>`).join('')}
function jExercises(m,sel=''){return (JOURNAL_LIBRARY[m]||[]).map(x=>`<option ${x===sel?'selected':''}>${esc(x)}</option>`).join('')}
function drawJournal(){
 const v=document.querySelector('#view'),all=jLoad().sort((a,b)=>(b.date||'').localeCompare(a.date||'')),last=all[0];
 if(journalDraft)return drawJournalEdit();
 v.innerHTML=`<div class="j-title"><div><small>LA FORGE · JOURNAL</small><h2>Carnet d'entraînement</h2></div><button id="jNew">+ Séance</button></div>
 ${last?`<section class="j-card"><small>DERNIÈRE SÉANCE · ${jDate(last.date)}</small><div class="j-stats"><b>${last.duration||0}<i>min</i></b><b>${jSets(last)}<i>séries</i></b><b>${jFmt(jTon(last))}<i>kg</i></b></div>${last.exercises.map(e=>`<div class="j-perf"><strong>${esc(e.name)}</strong><span>${e.sets.map(s=>`${jFmt(s.kg)}×${+s.reps||0}`).join(' · ')}</span></div>`).join('')}<button id="jResume" class="j-wide">Reprendre cette séance</button></section>`:`<section class="j-card j-empty">Aucune séance enregistrée.<br><span>Le carnet papier reste ton outil pendant l'entraînement.</span></section>`}
 <section class="j-card"><div class="j-section">Historique <span>${all.length}</span></div>${all.length?all.map(s=>`<button class="j-history" data-jopen="${s.id}"><span><b>${jDate(s.date)}</b><small>${s.exercises.map(e=>e.name).slice(0,2).join(' · ')}</small></span><span><b>${jFmt(jTon(s))} kg</b><small>${s.duration||0} min</small></span></button>`).join(''):'<div class="j-muted">Ton historique apparaîtra ici.</div>'}</section>`;
 document.querySelector('#jNew').onclick=()=>{journalDraft={id:jId(),date:jToday(),duration:0,notes:'',exercises:[]};drawJournalEdit()};
...
function drawJournalEdit(readOnly=false){
 const v=document.querySelector('#view'),d=journalDraft,prev=jPrev(d);
 v.innerHTML=`<div class="j-edit-head"><button id="jBack">‹ Journal</button><b>${readOnly?'Séance enregistrée':'Saisie de séance'}</b></div>
 ...
 ${prev?`<section class="j-card j-ref"><small>PERFORMANCES PRÉCÉDENTES · ${jDate(prev.date)}</small>${prev.exercises.map(e=>`<div class="j-perf"><strong>${esc(e.name)}</strong><span>${e.sets.map(s=>`${jFmt(s.kg)}×${s.reps}`).join(' · ')}</span></div>`).join('')}</section>`:''}
 ...
 ${readOnly?`<button id="jDelete" class="j-delete">Supprimer</button>`:`<button id="jValidate" class="j-validate">Valider la séance</button>`}`;
 document.querySelector('#jBack').onclick=()=>{journalDraft=null;drawJournal()};
 if(readOnly){document.querySelector('#jDelete').onclick=()=>{if(confirm('Supprimer cette séance ?')){jSave(jLoad().filter(x=>x.id!==d.id));journalDraft=null;drawJournal()}};return}
 ...
 v.querySelectorAll('.j-ex').forEach(card=>{const i=+card.dataset.i,e=d.exercises[i];
  card.querySelector('[data-muscle]').onchange=x=>{e.muscle=x.target.value;e.name=JOURNAL_LIBRARY[e.muscle][0];drawJournalEdit()};
  card.querySelector('[data-name]').onchange=x=>e.name=x.target.value;
  card.querySelector('[data-rmex]').onclick=()=>{d.exercises.splice(i,1);drawJournalEdit()};
  card.querySelector('[data-addset]').onclick=()=>{const q=e.sets.at(-1)||{kg:0,reps:0};e.sets.push({...q});drawJournalEdit()};
  card.querySelectorAll('.j-set').forEach(r=>{const k=+r.dataset.k,s=e.sets[k];r.querySelector('[data-kg]').oninput=x=>{s.kg=Math.max(0,+x.target.value||0);refresh()};r.querySelector('[data-reps]').oninput=x=>{s.reps=Math.max(0,+x.target.value||0);refresh()};r.querySelector('[data-rmset]').onclick=()=>{e.sets.splice(k,1);if(!e.sets.length)e.sets=[{kg:0,reps:0}];drawJournalEdit()}});
 });
 document.querySelector('#jAddEx').onclick=()=>{const m=Object.keys(JOURNAL_LIBRARY)[0];d.exercises.push({id:jId(),muscle:m,name:JOURNAL_LIBRARY[m][0],sets:[{kg:0,reps:0}]});drawJournalEdit()};
 document.querySelector('#jValidate').onclick=()=>{d.date=document.querySelector('#jDate').value||jToday();d.duration=Math.max(0,+document.querySelector('#jDur').value||0);d.notes=document.querySelector('#jNotes').value||'';d.exercises=d.exercises.filter(e=>e.name&&e.sets.some(s=>(+s.kg||0)||(+s.reps||0)));if(!d.exercises.length){alert('Renseigne au moins une série.');return}const a=jLoad(),i=a.findIndex(x=>x.id===d.id);i>=0?a[i]=d:a.push(d);jSave(a);journalDraft=null;drawJournal()};
}
function jExHtml(e,i,ro){
 return `<section class="j-card j-ex" data-i="${i}"><div class="j-exhead"><label>Groupe<select data-muscle ${ro?'disabled':''}>${jMuscles(e.muscle)}</select></label><label>Exercice<select data-name ${ro?'disabled':''}>${jExercises(e.muscle,e.name)}</select></label>${ro?'':`<button data-rmex>×</button>`}</div><div class="j-labels"><span>#</span><span>Charge kg</span><span>Reps</span><span></span></div>${e.sets.map((s,k)=>`<div class="j-set" data-k="${k}"><b>${k+1}</b><input data-kg type="number" inputmode="decimal" step=".5" min="0" value="${s.kg||''}" ${ro?'disabled':''}><input data-reps type="number" inputmode="numeric" min="0" value="${s.reps||''}" ${ro?'disabled':''}>${ro?'<span></span>':`<button data-rmset>×</button>`}</div>`).join('')}${ro?'':`<button data-addset class="j-addset">+ Série</button>`}</section>`
}
...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v182.png" alt="" aria-hidden="true">
      <div class="auth-brand">FORGELAB</div>
      <div class="auth-sub">ATLAS · ESPACE PERSONNEL</div>
      <div class="auth-version"><span>v1.8.2</span></div>
    </div>

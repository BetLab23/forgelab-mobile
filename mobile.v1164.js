...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1164.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 let ref=null,laurel=null,footerScene=null,posterRef=null;try{ref=await load('template-reference-v1164.png')}catch(_){ }try{laurel=await load('auth-laurel-realistic-v1164.png')}catch(_){ }try{footerScene=await load('footer-mountains-man-v1164.png')}catch(_){ }try{posterRef=await load('poster-reference-v1164.png')}catch(_){ }
 const atlasFiles={pectoraux:'atlas-pectoraux-v1164.png',dos:'atlas-dos-v1164.png',deltoide_anterieur:'atlas-deltoide_anterieur-v1164.png',deltoide_lateral:'atlas-deltoide_lateral-v1164.png',deltoide_posterieur:'atlas-deltoide_posterieur-v1164.png',trapezes_superieurs:'atlas-trapezes_superieurs-v1164.png',trapezes_moyens:'atlas-trapezes_moyens-v1164.png',cou:'atlas-cou-v1164.png',biceps:'atlas-biceps-v1164.png',triceps:'atlas-triceps-v1164.png',avant_bras:'atlas-avant_bras-v1164.png',abdominaux:'atlas-abdominaux-v1164.png',lombaires:'atlas-lombaires-v1164.png',fessiers:'atlas-fessiers-v1164.png',adducteurs:'atlas-adducteurs-v1164.png',quadriceps:'atlas-quadriceps-v1164.png',ischios:'atlas-ischios-v1164.png',mollets:'atlas-mollets-v1164.png'};
 ...
 // Header v1.16.4 : zone entièrement reconstruite pour supprimer tout élément fantôme du template.
 ...
 // v1.16.4 — composition éditoriale ForgeLab : pas d'icônes UI, pas de cartes.
 ...
 const anatomy=(cx,cy,names,accent)=>{
   // v1.16.4: afficher TOUS les groupes travaillés, au lieu du seul premier groupe.
 ...
 // Footer v1.16.4 : scène continue + signature DAILY WINS. dans le ciel.
  ...
  const rows=DAYS.map(day=>{
    ...
    const items=entries.length?entries.map(x=>{
      ...
      return `<div class="mobile-program-entry" data-id="${esc(x.id)}">
        ...
        <div class="program-exercises" data-program-exercises hidden>
          ${(()=>{const ex=x.exercises||[],used=ex.reduce((a,e)=>a+Number(e.sets||0),0),diff=Number(x.sets||0)-used,cl=diff===0?'ok':diff>0?'warn':'over';return `<div class="program-exercise-status ${cl}"><b>${used}/${Number(x.sets||0)} séries attribuées</b><span>${diff===0?'Complet':diff>0?diff+' à programmer':Math.abs(diff)+' en trop'}</span></div>${ex.map(e=>`<div class="program-exercise-row" data-exercise-id="${esc(e.id)}"><input data-ex-name value="${esc(e.name)}" aria-label="Exercice"><input data-ex-sets type="number" min="1" max="20" value="${Number(e.sets||1)}" aria-label="Séries"><select data-ex-rest aria-label="Repos">${[30,45,60,75,90,120,150,180,240].map(r=>`<option value="${r}" ${Number(e.rest||90)===r?'selected':''}>${r<60?r+' s':(r/60)+' min'}</option>`).join('')}</select><button type="button" data-ex-remove aria-label="Supprimer">×</button></div>`).join('')}<div class="program-exercise-add"><input data-ex-new-name placeholder="Ajouter un exercice"><input data-ex-new-sets type="number" min="1" max="20" value="1" aria-label="Séries"><select data-ex-new-rest>${[60,90,120,150,180].map(r=>`<option value="${r}" ${r===90?'selected':''}>${r/60} min</option>`).join('')}</select><button type="button" data-ex-add>Ajouter</button></div>`})()}
        </div>
      </div>`;
...
v.querySelector('[data-program-image]')?.addEventListener('click',generateProgramImage);
  ...
  v.querySelectorAll('.mobile-program-day').forEach(card=>{
    ...
    card.querySelectorAll('.mobile-program-entry').forEach(row=>{
      ...
      row.querySelector('.mobile-program-muscle')?.addEventListener('click',()=>{const box=row.querySelector('[data-program-exercises]');box.hidden=!box.hidden});
      ...
      row.querySelector('[data-ex-add]')?.addEventListener('click',()=>{const name=row.querySelector('[data-ex-new-name]').value.trim();if(!name)return;const sets=Math.max(1,+row.querySelector('[data-ex-new-sets]').value||1),rest=Math.max(15,+row.querySelector('[data-ex-new-rest]').value||90);mutateExercises(ex=>ex.push({id:newProgramId(),name,sets,rest}))});
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1164.png" alt=""><strong>ForgeLab</strong></div>
...
/* ===== ForgeLab v1.16.4 — Programme complet / multi-blocs ===== */
...
function addFullBlock(){
 ...
 const name=(prompt('Nom du nouveau bloc :',`Bloc ${n}`)||'').trim();if(!name)return;
...
function generateCompleteProgram(){
 saveCurrentIntoBlock();const fp=fullProgram();
 const esc2=s=>String(s??'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]));
 let body=`<h1>${esc2(fp.name||'PROGRAMME FORGELAB')}</h1><p class="sub">PROGRAMME COMPLET · ${fp.blocks.length} BLOC${fp.blocks.length>1?'S':''}</p>`;
 fp.blocks.forEach((b,bi)=>{const {meta,phases}=blockReadableData(b.id);body+=`<section><h2>${bi+1}. ${esc2(b.name||meta.name)}</h2><div class="meta">${meta.duration} semaines · trame ${meta.rhythm} + deload</div>`;phases.forEach(({q,data})=>{body+=`<h3>${q.label} · S${q.from}${q.to!==q.from?'–S'+q.to:''}</h3><div class="days">`;DAYS.forEach(d=>{const es=(data[d]||[]).filter(z=>+z.sets>0);if(!es.length)return;body+=`<article><b>${d}</b>`+es.map(z=>`<div><strong>${esc2(z.muscle_name)}</strong> · ${z.sets} séries${(z.exercises||[]).length?'<ul>'+z.exercises.map(e=>`<li>${esc2(e.name)} — ${e.sets} séries · repos ${e.rest}s</li>`).join('')+'</ul>':''}</div>`).join('')+`</article>`});body+=`</div>`});body+=`</section>`});
 const w=window.open('','_blank');if(!w)return alert('Autorise les fenêtres surgissantes pour générer le programme complet.');
 w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>ForgeLab — Programme complet</title><style>body{font-family:Arial;background:#080806;color:#f5efe2;margin:0;padding:32px}h1{text-align:center;color:#e2bd70;letter-spacing:2px}.sub{text-align:center;color:#aaa}section{border:1px solid #8b6d38;padding:20px;margin:24px 0;break-inside:avoid}h2,h3{color:#e2bd70}.meta{color:#bbb;margin-bottom:18px}.days{display:grid;grid-template-columns:repeat(2,1fr);gap:12px}article{border:1px solid #3c3424;padding:12px;background:#11100c}article>b{color:#e2bd70}ul{margin:6px 0 0 18px;color:#ccc}@media print{body{background:#fff;color:#111}section,article{border-color:#777;background:#fff}h1,h2,h3,article>b{color:#7a5b20}.sub,.meta,ul{color:#333}}</style></head><body>${body}<script>setTimeout(()=>window.print(),400)<\/script></body></html>`);w.document.close()
}

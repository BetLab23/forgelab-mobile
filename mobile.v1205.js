...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1205.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 const assets={};for(const f of ['auth-laurel-realistic-v1205.png','footer-mountains-man-v1205.png']){try{assets[f]=await load(f)}catch(_){}}
 ...
 // Fond noir métal + halo or, sans anatomie.
 const bg=g.createLinearGradient(0,0,0,H);bg.addColorStop(0,'#030405');bg.addColorStop(.48,'#0a0c0d');bg.addColorStop(1,'#020303');g.fillStyle=bg;g.fillRect(0,0,W,H);
 const glow=g.createRadialGradient(W/2,90,20,W/2,90,620);glow.addColorStop(0,'rgba(229,187,104,.18)');glow.addColorStop(1,'rgba(0,0,0,0)');g.fillStyle=glow;g.fillRect(0,0,W,520);
 // Header compact pensé pour un seul écran iPhone.
 const laurel=assets['auth-laurel-realistic-v1205.png'];if(laurel)g.drawImage(laurel,W/2-46,18,92,80);
 txt('FORGELAB',W/2,104,27,gold2,900,'center');sans('DISCIPLINA · CONSTANTIA · PROGRESSUS',W/2,137,11,'rgba(245,241,233,.72)',600,'center');
 fit((fp.name||'PROGRAMME FORGELAB').toUpperCase(),W/2,188,W-100,48,gold2,900,'center',true);
 let range='';try{range=selected&&typeof fl176RangeLabel==='function'?fl176RangeLabel(selected):''}catch(_){}
 const sub=[blockName.toUpperCase(),range].filter(Boolean).join(' · ');fit(sub,W/2,232,W-120,22,white,800,'center',false);
 const totalSets=activeDays.reduce((a,d)=>a+(p[d]||[]).reduce((s,e)=>s+(+e.sets||0),0),0);
 sans(`${activeDays.length} JOURS · ${totalSets} SÉRIES`,W/2,264,13,gold,800,'center');
 g.strokeStyle='rgba(228,187,104,.55)';g.beginPath();g.moveTo(48,290);g.lineTo(W-48,290);g.stroke();
 // Grille dynamique 2 colonnes. Le contenu, pas une illustration, est prioritaire.
 const cols=activeDays.length===1?1:2,rows=Math.ceil(activeDays.length/cols),margin=30,gap=14,top=310,footerH=112,bottom=footerH+18;
 ...
 activeDays.forEach((day,i)=>{
   ...
   g.fillStyle='rgba(8,10,11,.95)';g.fillRect(x,y,cardW,cardH);g.strokeStyle='rgba(228,187,104,.70)';g.lineWidth=1.4;g.strokeRect(x+.7,y+.7,cardW-1.4,cardH-1.4);
   const headH=Math.max(46,Math.min(58,cardH*.13));g.fillStyle='rgba(228,187,104,.055)';g.fillRect(x,y,cardW,headH);
   txt(day.toUpperCase(),x+18,y+headH/2,Math.min(24,headH*.45),gold2,900);sans(`${daySets} SÉRIES`,x+cardW-16,y+headH/2,12,gold,800,'right');
 ...
 const footer=assets['footer-mountains-man-v1205.png'];if(footer){g.save();g.globalAlpha=.46;const fw=W,fh=150,sy=Math.max(0,footer.height-footer.width*(fh/fw));g.drawImage(footer,0,sy,footer.width,footer.height-sy,0,H-footerH,fw,footerH);g.restore()}
 ...
 txt('DAILY WINS.',W/2,H-39,18,gold2,900,'center');
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1205.png" alt=""><strong>ForgeLab</strong></div>
...
/* ===== ForgeLab v1.20.5 — Brouillon structuré : Programme > Trame > Blocs > Génération ===== */

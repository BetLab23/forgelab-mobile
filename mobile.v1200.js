...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1200.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 const assets={};
 const files=['auth-laurel-realistic-v1200.png'];
 ...
 const laurel=assets['auth-laurel-realistic-v1200.png'];if(laurel)g.drawImage(laurel,W/2-56,28,112,88);
 ...
 function drawAnatomy(keys,x,y,w,h){
   // v1.20.0 — atlas vectoriel natif : aucune image/aucun masque PNG.
   // Toutes les zones sont définies dans le même repère anatomique, donc aucun décalage possible.
   const active=new Set(keys);
   const GOLD='#d7aa4f', GOLD_EDGE='#f2cb73', BODY='#303235', EDGE='#716b60', DETAIL='rgba(205,198,184,.24)';
   ...
   const poly=(ox,pts,fill=BODY,stroke=EDGE,lw=1)=>{const a=P(ox,pts);ctx.beginPath();a.forEach((p,i)=>i?ctx.lineTo(...p):ctx.moveTo(...p));ctx.closePath();ctx.fillStyle=fill;ctx.fill();if(stroke){ctx.strokeStyle=stroke;ctx.lineWidth=lw;ctx.stroke()}};
   const ell=(ox,cx,cy,rx,ry,fill=BODY,stroke=EDGE,lw=1)=>{ctx.beginPath();ctx.ellipse(ox+cx*scale,oy+cy*scale,rx*scale,ry*scale,0,0,Math.PI*2);ctx.fillStyle=fill;ctx.fill();if(stroke){ctx.strokeStyle=stroke;ctx.lineWidth=lw;ctx.stroke()}};
   const line=(ox,pts)=>{const a=P(ox,pts);ctx.beginPath();a.forEach((p,i)=>i?ctx.lineTo(...p):ctx.moveTo(...p));ctx.strokeStyle=DETAIL;ctx.lineWidth=.8;ctx.stroke()};
   const hiPoly=(ox,k,pts)=>{if(!active.has(k))return;ctx.save();ctx.shadowColor='rgba(242,203,115,.25)';ctx.shadowBlur=4;poly(ox,pts,GOLD,GOLD_EDGE,1);ctx.restore()};
   const hiEll=(ox,k,cx,cy,rx,ry)=>{if(!active.has(k))return;ctx.save();ctx.shadowColor='rgba(242,203,115,.25)';ctx.shadowBlur=4;ell(ox,cx,cy,rx,ry,GOLD,GOLD_EDGE,1);ctx.restore()};
   const front=(ox)=>{
     // silhouette
     ell(ox,50,14,8,10); poly(ox,[[43,24],[57,24],[61,33],[70,39],[73,58],[68,82],[64,106],[60,124],[58,145],[56,169],[55,204],[47,204],[46,170],[43,145],[40,124],[36,106],[32,82],[27,58],[30,39],[39,33]]);
     poly(ox,[[30,39],[23,45],[19,67],[18,91],[23,93],[28,70],[34,52]]); poly(ox,[[70,39],[77,45],[81,67],[82,91],[77,93],[72,70],[66,52]]);
     poly(ox,[[18,91],[16,116],[20,119],[25,94]]); poly(ox,[[82,91],[84,116],[80,119],[75,94]]);
     // anatomical detail
     line(ox,[[50,27],[50,121]]); line(ox,[[37,55],[50,58],[63,55]]); line(ox,[[39,70],[50,73],[61,70]]); line(ox,[[42,84],[58,84]]); line(ox,[[43,94],[57,94]]); line(ox,[[43,104],[57,104]]); line(ox,[[40,124],[50,130],[60,124]]); line(ox,[[46,145],[54,145]]); line(ox,[[46,170],[54,170]]);
     // highlights FRONT
   ...
   const back=(ox)=>{
     ...
     poly(ox,[[30,39],[23,45],[19,67],[18,91],[23,93],[28,70],[34,52]]); poly(ox,[[70,39],[77,45],[81,67],[82,91],[77,93],[72,70],[66,52]]);
     poly(ox,[[18,91],[16,116],[20,119],[25,94]]); poly(ox,[[82,91],[84,116],[80,119],[75,94]]);
     line(ox,[[50,26],[50,122]]); line(ox,[[36,45],[50,51],[64,45]]); line(ox,[[36,61],[50,67],[64,61]]); line(ox,[[38,83],[50,78],[62,83]]); line(ox,[[40,105],[50,111],[60,105]]); line(ox,[[40,124],[50,132],[60,124]]); line(ox,[[46,145],[54,145]]); line(ox,[[46,170],[54,170]]);
     // highlights BACK
   ...
   const pairW=drawW*2+pairGap, start=x+(w-pairW)/2;
   front(start); back(start+drawW+pairGap);
   // labels discrets pour éviter toute ambiguïté de lecture
   ctx.fillStyle='rgba(240,198,109,.62)';ctx.font=`700 ${Math.max(8,8*scale)}px Arial`;ctx.textAlign='center';ctx.textBaseline='bottom';
   ctx.fillText('AVANT',start+drawW/2,oy+drawH);ctx.fillText('ARRIÈRE',start+drawW+pairGap+drawW/2,oy+drawH);
 }
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1200.png" alt=""><strong>ForgeLab</strong></div>
...
/* ===== ForgeLab v1.20.0 — Brouillon structuré : Programme > Trame > Blocs > Génération ===== */

...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1138.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 let ref=null,laurel=null,footerScene=null;try{ref=await load('template-reference-v1138.png')}catch(_){ }try{laurel=await load('auth-laurel-realistic-v1138.png')}catch(_){ }try{footerScene=await load('footer-mountains-man-v1138.png')}catch(_){ }
 ...
 // Footer v1.13.8 : une seule scène continue, sans raccord ni inscription.
 ...
 if(footerScene){
  ...
  // v1.13.8 — silhouette plus longiligne : on affine fortement les mollets
  // directement au rendu Canvas, sans modifier ni générer l'image source.
  // Coordonnées exprimées dans l'image source puis projetées dans le footer.
  const sxScale=1024/fw, syScale=footerH/cropH;
  const mapX=sx=>sx*sxScale;
  const mapY=sy=>1224+(sy-cropY)*syScale;
  const mapW=sw=>sw*sxScale;
  const mapH=sh=>sh*syScale;
  const slimLeg=(sx,sy,sw,sh,neighborX)=>{
    // Efface l'ancien mollet avec une bande du décor immédiatement voisin.
    g.drawImage(footerScene,neighborX,sy,sw,sh,mapX(sx),mapY(sy),mapW(sw),mapH(sh));
    // Réinjecte le centre du mollet à ~52 % de sa largeur d'origine.
    const srcInset=Math.round(sw*.24), srcW=sw-srcInset*2;
    const dstW=mapW(sw*.52), dstX=mapX(sx)+((mapW(sw)-dstW)/2);
    g.drawImage(footerScene,sx+srcInset,sy,srcW,sh,dstX,mapY(sy),dstW,mapH(sh));
  };
  // Personnage situé à gauche du paysage : traitement limité sous les genoux.
  slimLeg(137,1337,19,58,118);
  slimLeg(162,1337,19,58,183);
 }else if(ref){
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1138.png" alt=""><strong>ForgeLab</strong></div>

...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1177.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 const W=1024,H=1536,SCALE=2,c=document.createElement('canvas');c.width=W*SCALE;c.height=H*SCALE;const g=c.getContext('2d');g.scale(SCALE,SCALE);
 const load=src=>new Promise((res,rej)=>{const im=new Image();im.onload=()=>res(im);im.onerror=rej;im.src=src});
 let ref=null,laurel=null,footerScene=null,posterRef=null;try{ref=await load('template-reference-v1177.png')}catch(_){ }try{laurel=await load('auth-laurel-realistic-v1177.png')}catch(_){ }try{footerScene=await load('footer-mountains-man-v1177.png')}catch(_){ }try{posterRef=await load('poster-reference-v1177.png')}catch(_){ }
 const atlasFiles={pectoraux:'atlas-pectoraux-v1177.png',dos:'atlas-dos-v1177.png',deltoide_anterieur:'atlas-deltoide_anterieur-v1177.png',deltoide_lateral:'atlas-deltoide_lateral-v1177.png',deltoide_posterieur:'atlas-deltoide_posterieur-v1177.png',trapezes_superieurs:'atlas-trapezes_superieurs-v1177.png',trapezes_moyens:'atlas-trapezes_moyens-v1177.png',cou:'atlas-cou-v1177.png',biceps:'atlas-biceps-v1177.png',triceps:'atlas-triceps-v1177.png',avant_bras:'atlas-avant_bras-v1177.png',abdominaux:'atlas-abdominaux-v1177.png',lombaires:'atlas-lombaires-v1177.png',fessiers:'atlas-fessiers-v1177.png',adducteurs:'atlas-adducteurs-v1177.png',quadriceps:'atlas-quadriceps-v1177.png',ischios:'atlas-ischios-v1177.png',mollets:'atlas-mollets-v1177.png'};
 const atlasImgs={};await Promise.all(Object.entries(atlasFiles).map(async([k,f])=>{try{atlasImgs[k]=await load(f)}catch(_){}}));
 if(ref)g.drawImage(ref,0,0,W,H);else{g.fillStyle='#03080c';g.fillRect(0,0,W,H)}
 const white='#f7f1e6',muted='#c7bda9',blue='#e1bd70',green='#f4dda3',grey='#a9a092',dark='rgba(10,9,6,.975)',panel='rgba(17,15,10,.975)';
 const ember='#b9944f',copper='#d7b66c',gold='#f2d58a',iron='#1d1a12';
 ...
 // Header v1.17.0 : zone entièrement reconstruite pour supprimer tout élément fantôme du template.
 // On recouvre volontairement toute la hauteur 0–220 avant de composer le nouveau bandeau.
 const headerBg=g.createLinearGradient(0,0,W,220);headerBg.addColorStop(0,'#070604');headerBg.addColorStop(.48,'#171309');headerBg.addColorStop(1,'#090806');g.fillStyle=headerBg;g.fillRect(0,0,W,220);
 const forgeGlow=g.createRadialGradient(512,62,0,512,62,420);forgeGlow.addColorStop(0,'rgba(240,210,138,.18)');forgeGlow.addColorStop(.55,'rgba(177,137,67,.06)');forgeGlow.addColorStop(1,'rgba(0,0,0,0)');g.fillStyle=forgeGlow;g.fillRect(0,0,W,220);
 // Laurier isolé et respirant.
 if(laurel)g.drawImage(laurel,466,8,92,72);
 // Filets métalliques sous le laurier.
 const metal=g.createLinearGradient(86,0,938,0);metal.addColorStop(0,'rgba(214,173,96,0)');metal.addColorStop(.18,'#6f5734');metal.addColorStop(.5,'#e6c77e');metal.addColorStop(.82,'#6f5734');metal.addColorStop(1,'rgba(214,173,96,0)');line(86,94,938,94,metal,1);
 // Cartouche titre plus fin : pas de grosse boîte, seulement une plaque sombre et des accents forge.
 const titleBg=g.createLinearGradient(0,106,0,196);titleBg.addColorStop(0,'rgba(25,21,12,.96)');titleBg.addColorStop(1,'rgba(9,8,6,.98)');g.fillStyle=titleBg;g.fillRect(62,108,900,88);
 g.strokeStyle='rgba(166,132,70,.72)';g.lineWidth=1;g.strokeRect(62.5,108.5,899,87);
 const accent=g.createLinearGradient(96,0,928,0);accent.addColorStop(0,'rgba(214,173,96,0)');accent.addColorStop(.22,copper);accent.addColorStop(.5,gold);accent.addColorStop(.78,copper);accent.addColorStop(1,'rgba(214,173,96,0)');line(96,116,928,116,accent,2);
 fit((meta.name||'PROGRAMME FORGELAB').toUpperCase(),512,148,850,38,white,700,'center');
 txt('—  PROGRAMMATION  —',512,181,11,gold,500,'center','Georgia');
 // Fin de header nette avant les métadonnées.
 line(24,211,1000,211,'#3a321f',1);
 // v1.17.0 — composition éditoriale ForgeLab : pas d'icônes UI, pas de cartes.
 box(20,220,984,88,'rgba(7,7,5,.995)');
 const info=[['DURÉE',`${meta.duration} SEMAINES`],['OBJECTIF','HYPERTROPHIE'],['FRÉQUENCE',`${activeDays.length} SÉANCE${activeDays.length>1?'S':''} / SEMAINE`],['TRAME',`${meta.rhythm} + DELOAD`]];
 line(38,232,986,232,'rgba(214,173,96,.45)',1);
 info.forEach((a,i)=>{const cx=146+i*244;txt(a[0],cx,254,9,'rgba(242,213,138,.78)',800,'center','Trebuchet MS');fit(a[1],cx,282,205,i===2?14:16,white,900,'center');if(i<3)line(268+i*244,246,268+i*244,288,'rgba(214,173,96,.20)',1)});
 line(38,298,986,298,'rgba(214,173,96,.20)',1);
 // Cycles compacts : identité de phase, sans grand panneau.
 box(18,312,988,190,'rgba(7,7,5,.995)');
 const gap=12,pw=(976-gap*(ph.length-1))/ph.length;
 ph.forEach((q,i)=>{const x=24+i*(pw+gap),cx=x+pw/2,ac=q.deload?grey:gold;line(x+12,330,cx-58,330,q.deload?'rgba(169,160,146,.32)':'rgba(214,173,96,.34)',1);line(cx+58,330,x+pw-12,330,q.deload?'rgba(169,160,146,.32)':'rgba(214,173,96,.34)',1);txt(q.deload?`DELOAD ${ph.slice(0,i+1).filter(z=>z.deload).length}`:`CYCLE ${roman(ph.slice(0,i+1).filter(z=>!z.deload).length)}`,cx,330,11,ac,850,'center','Trebuchet MS');txt(`S${q.from}${q.to!==q.from?' – S'+q.to:''}`,cx,373,21,white,900,'center');txt(q.deload?'RÉCUPÉRATION':(i===0?'DÉVELOPPEMENT':i===ph.length-1?'SPÉCIALISATION':'INTENSIFICATION'),cx,414,11,ac,800,'center','Trebuchet MS');line(cx-28,435,cx+28,435,q.deload?'rgba(169,160,146,.35)':'rgba(214,173,96,.40)',1);fit(q.deload?'Volume réduit · récupération':'Progression · technique · qualité',cx,462,pw-20,10,muted,500,'center')});
 ...
 const atlasKey=name=>{const n=(name||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[- ]/g,'_');
   if(/delto.*anter|epaule.*anter/.test(n))return'deltoide_anterieur';
   if(/delto.*later|epaule.*later/.test(n))return'deltoide_lateral';
   if(/delto.*post|epaule.*post/.test(n))return'deltoide_posterieur';
   if(/trap.*super/.test(n))return'trapezes_superieurs';
   if(/trap.*moy/.test(n))return'trapezes_moyens';
   if(/pect/.test(n))return'pectoraux';if(/^dos$|dors/.test(n))return'dos';if(/cou/.test(n))return'cou';
   if(/biceps/.test(n))return'biceps';if(/triceps/.test(n))return'triceps';if(/avant.*bras/.test(n))return'avant_bras';
   if(/abdo/.test(n))return'abdominaux';if(/lomb/.test(n))return'lombaires';if(/fess/.test(n))return'fessiers';
   if(/adduct/.test(n))return'adducteurs';if(/quad/.test(n))return'quadriceps';if(/isch/.test(n))return'ischios';if(/mollet/.test(n))return'mollets';return null};
 const anatomy=(cx,cy,names,accent)=>{
   // v1.17.0: afficher TOUS les groupes travaillés, au lieu du seul premier groupe.
   // Les atlas actuels sont des planches anatomiques finies (pas des calques superposables),
   // donc on conserve leur anatomie exacte et on les compose sans les déformer.
   const keys=[...new Set(names.map(atlasKey).filter(Boolean))].slice(0,5);
   const ims=keys.map(k=>atlasImgs[k]).filter(Boolean);
   if(!ims.length)return;
   const areaW=cardW-16, areaH=172;
   const cols=ims.length<=2?ims.length:Math.min(3,ims.length), rows=Math.ceil(ims.length/cols);
   const cellW=areaW/cols, cellH=areaH/rows;
   ims.forEach((im,i)=>{
     const iw=im.naturalWidth||im.width,ih=im.naturalHeight||im.height;
     const sc=Math.min((cellW-4)/iw,(cellH-4)/ih),dw=iw*sc,dh=ih*sc;
     const col=i%cols,row=Math.floor(i/cols),x=cx-areaW/2+col*cellW+(cellW-dw)/2,y=cy-areaH/2+row*cellH+(cellH-dh)/2;
     g.save();g.imageSmoothingEnabled=true;g.imageSmoothingQuality='high';g.globalAlpha=1;g.drawImage(im,x,y,dw,dh);g.restore();
   });
 };
 ...
 // Footer v1.17.0 : scène continue + signature DAILY WINS. dans le ciel.
 ...
 // Signature ForgeLab : placée dans le ciel, au-dessus des montagnes, sans toucher au panorama.
 txt('DAILY WINS.',512,1272,22,'rgba(242,213,138,.94)',900,'center');
 line(446,1290,578,1290,'rgba(214,173,96,.52)',1);
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1177.png" alt=""><strong>ForgeLab</strong></div>
...
function drawProgrammingChooser(){
 ...
 v.innerHTML=title('Programmation')+`<section class="program-choice-screen">
   ...
   <div class="program-draft-list">${drafts.map((d,i)=>`<button type="button" class="program-choice-card draft" data-program-draft-id="${esc(d.id)}"><span class="program-choice-icon">${roman(i+1)}</span><div><small>BROUILLON ${roman(i+1)}</small><strong>${esc(d.name||'Programme en préparation')}</strong><span>Programme indépendant · modifiable sans toucher à l’actif</span></div><b>›</b></button>`).join('')}</div>
...
/* ===== ForgeLab v1.17.7 — Brouillon structuré : Programme > Trame > Blocs > Génération ===== */
let programBlockOpen=false;
function fl176TrameCount(fp){return Math.max(1,Number(fp?.trameBlocks||3))}
function fl176Schedule(totalDuration,blockCount){
  const n=Math.max(1,Number(blockCount)||3), total=Math.max(n*2,Number(totalDuration)||n*5);
  const trainingWeeks=total-n, base=Math.floor(trainingWeeks/n), extra=trainingWeeks%n;
  const out=[];let week=1;
  for(let i=0;i<n;i++){
    const len=Math.max(1,base+(i<extra?1:0)),from=week,to=week+len-1;
    out.push({kind:'block',index:i,from,to});week=to+1;
    out.push({kind:'deload',index:i,from:week,to:week});week++;
  }
...
drawProgramming=function(){
  ...
  const mode=programmingMode();if(mode!=='draft'){
    ...
    const back=document.createElement('button');back.type='button';back.className='program-back-choice';back.innerHTML='‹ <span>Programmation</span>';back.onclick=()=>{programmingScreen='chooser';drawProgramming()};const h=v.querySelector('.section-title');h?h.insertAdjacentElement('afterend',back):v.prepend(back);return;
  }
  let fp=fl176EnsureStructure(),sel=selectedFullBlock();
  ...
  const config=document.createElement('section');config.className='fl176-panel fl176-program-config';config.innerHTML=`<div class="fl176-panel-title"><small>I · PROGRAMME COMPLET</small><strong>Structure du programme</strong></div><div class="fl176-config-grid"><label><span>Nom du programme</span><input data-fl176-name value="${esc(fp.name||'Programme ForgeLab')}"></label><label><span>Durée totale</span><div class="fl176-duration"><input type="number" min="1" max="52" data-fl176-duration value="${Number(fp.totalDuration||14)}"><b>semaines</b></div></label><label class="wide"><span>Trame</span><select data-fl176-trame>${[2,3,4,5,6].map(n=>`<option value="${n}" ${n===fl176TrameCount(fp)?'selected':''}>${n} blocs + deload après chaque bloc</option>`).join('')}</select></label></div><div class="fl176-trame-preview">${fp.blocks.map(b=>`<span class="${b.deload?'deload':''}">${b.deload?`DELOAD ${roman(b.blockIndex)}`:`BLOC ${roman(b.blockIndex)}`}<b>${fl176RangeLabel(b)}</b></span>`).join('<i>›</i>')}</div>`;
  const blocks=document.createElement('section');blocks.className='fl176-panel fl176-block-manager';blocks.innerHTML=`<div class="fl176-panel-title"><small>II · PARAMÉTRAGE DES BLOCS</small><strong>Choisis un bloc à programmer</strong></div><div class="fl176-block-list">${fp.blocks.map((b,i)=>{const ok=fl176BlockComplete(b.id),active=b.id===fp.selected&&programBlockOpen;return `<button type="button" class="fl176-block-card ${b.deload?'deload':''} ${active?'open':''}" data-fl176-block="${esc(b.id)}"><span><small>${b.deload?`DELOAD ${roman(b.blockIndex)}`:`BLOC ${roman(b.blockIndex)}`}</small><strong>${esc(b.name)} · ${fl176RangeLabel(b)}</strong></span><em class="${ok?'ok':''}">${ok?'Paramétré':'À paramétrer'}</em><b>${active?'⌃':'⌄'}</b></button>`}).join('')}</div>${programBlockOpen?`<div class="fl176-open-block"><div><small>${sel.deload?`DELOAD ${roman(sel.blockIndex)}`:`BLOC ${roman(sel.blockIndex)}`} · ${fl176RangeLabel(sel)}</small>${sel.deload?`<strong>Deload</strong>`:`<label>Nom du bloc<input data-fl176-block-name value="${esc(sel.name)}"></label>`}</div><span>La programmation quotidienne de ce bloc est ouverte ci-dessous.</span></div>`:''}`;
  const generation=document.createElement('section');generation.className='fl176-panel fl176-generation';const statuses=fp.blocks.map(b=>fl176BlockComplete(b.id)),allOk=statuses.every(Boolean);generation.innerHTML=`<div class="fl176-panel-title"><small>III · GÉNÉRATION</small><strong>Images du programme</strong></div><div class="fl176-generation-list">${fp.blocks.map((b,i)=>`<div><span><small>${b.deload?`DELOAD ${roman(b.blockIndex)}`:`BLOC ${roman(b.blockIndex)}`}</small><strong>${esc(b.name)} · ${fl176RangeLabel(b)}</strong></span><button type="button" data-fl176-generate="${esc(b.id)}" ${statuses[i]?'':'disabled'}>Générer</button></div>`).join('')}</div><button type="button" class="fl176-generate-complete" data-fl176-complete ${allOk?'':'disabled'}>Générer le programme complet</button>${allOk?'':`<p>Le programme complet sera disponible lorsque tous les blocs et le deload seront paramétrés.</p>`}`;
  const anchor=legacySteps||v.firstChild;v.insertBefore(config,anchor);v.insertBefore(blocks,anchor);
  // Hide daily programming until a block is explicitly opened. Keep generation below it.
  const dailyNodes=[legacySteps,v.querySelector('.program-mobile-intro'),v.querySelector('.program-mobile-actions'),v.querySelector('.program-mobile-library'),v.querySelector('.program-mobile-note'),v.querySelector('.program-mobile-kpis'),v.querySelector('.program-mobile-days'),v.querySelector('.program-mobile-summary'),v.querySelector('.program-mobile-grid'),v.querySelector('.program-summary')].filter(Boolean);
  dailyNodes.forEach(n=>{n.hidden=!programBlockOpen;n.style.display=programBlockOpen?'':'none'});
  if(dailyNodes.length){const last=dailyNodes[dailyNodes.length-1];last.insertAdjacentElement('afterend',generation)}else v.appendChild(generation);
  ...
  config.querySelector('[data-fl176-duration]').onchange=e=>{fp=fullProgram();const min=fl176TrameCount(fp)*2;fp.totalDuration=Math.max(min,Math.min(52,Number(e.target.value)||14));saveFullProgram(fp);fl176EnsureStructure();programBlockOpen=false;drawProgramming()};
  ...
  generation.querySelectorAll('[data-fl176-generate]').forEach(b=>b.onclick=()=>fl176GenerateBlock(b.dataset.fl176Generate));
  generation.querySelector('[data-fl176-complete]')?.addEventListener('click',()=>{saveCurrentIntoBlock();generateCompleteProgram()});
};

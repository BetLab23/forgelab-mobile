...
function defaultCardio(){return {target:2,weeks:{}}}
...
function drawCardio(){
  ...
  const {data,list}=cardioWeek();
  const done=list.filter(x=>x.done).length;
  const minutes=list.reduce((a,x)=>a+(+x.duration||0),0);
  v.innerHTML=title('Cardio')+
  `<section class="cardio-overview">
    <div class="cardio-overview-head"><div><small>OBJECTIF HEBDOMADAIRE</small><strong>S${state.week} · ${done}/${data.target} réalisées</strong></div><b>${minutes}<span> min</span></b></div>
    <div class="cardio-target"><label>Séances prévues</label><div class="stepper"><button data-cardio-minus>−</button><strong>${data.target}</strong><button data-cardio-plus>+</button></div></div>
    <p>Ajoute seulement les séances prévues. Le type d’effort est choisi pour chaque séance : les autres exercices restent masqués.</p>
  </section>`+
  `<div class="cardio-list">${list.length?list.map((s,i)=>cardioSessionHtml(s,i)).join(''):`<div class="cardio-empty">Aucune séance cardio programmée en S${state.week}.</div>`}</div>`+
  `<button class="cardio-add" data-cardio-add>＋ Ajouter une séance cardio</button>`;

  v.querySelector('[data-cardio-minus]').onclick=()=>setCardioTarget(Math.max(0,data.target-1));
  v.querySelector('[data-cardio-plus]').onclick=()=>setCardioTarget(Math.min(7,data.target+1));
  v.querySelector('[data-cardio-add]').onclick=addCardioSession;
  v.querySelectorAll('[data-cardio-field]').forEach(el=>el.onchange=()=>updateCardioSession(+el.dataset.i,el.dataset.cardioField,el.type==='checkbox'?el.checked:el.value));
  v.querySelectorAll('[data-cardio-delete]').forEach(el=>el.onclick=()=>deleteCardioSession(+el.dataset.cardioDelete));
}
function cardioSessionHtml(s,i){
  return `<article class="card cardio-session ${s.done?'cardio-done':''}">
    <div class="cardio-session-head"><div><small>SÉANCE ${i+1}</small><strong>${esc(s.type||CARDIO_TYPES[0])}</strong></div><button class="cardio-delete" data-cardio-delete="${i}" aria-label="Supprimer">×</button></div>
    <div class="cardio-grid">
      <label class="wide">Type<select data-cardio-field="type" data-i="${i}">${cardioTypeOptions(s.type||CARDIO_TYPES[0])}</select></label>
      <label>Jour<select data-cardio-field="day" data-i="${i}">${DAYS.map((d,j)=>`<option value="${j+1}" ${+(s.day||1)===j+1?'selected':''}>${d}</option>`).join('')}</select></label>
      <label>Durée<input data-cardio-field="duration" data-i="${i}" type="number" inputmode="numeric" min="0" max="240" value="${+s.duration||20}"><span class="field-unit">min</span></label>
      <label class="wide">Intensité<select data-cardio-field="intensity" data-i="${i}">${cardioIntensityOptions(s.intensity||CARDIO_INTENSITIES[0])}</select></label>
    </div>
    <label class="cardio-check"><input data-cardio-field="done" data-i="${i}" type="checkbox" ${s.done?'checked':''}><span>${s.done?'Séance réalisée':'Marquer comme réalisée'}</span></label>
  </article>`
}
function setCardioTarget(n){const d=cardioData();d.target=n;saveCardio(d);drawCardio()}
function addCardioSession(){const {data,list}=cardioWeek();list.push({type:CARDIO_TYPES[0],day:1,duration:20,intensity:CARDIO_INTENSITIES[0],done:false});saveCardio(data);drawCardio()}
function updateCardioSession(i,field,value){const {data,list}=cardioWeek();if(!list[i])return;if(field==='duration')value=Math.max(0,Math.min(240,+value||0));if(field==='day')value=+value||1;if(field==='done')value=!!value;list[i][field]=value;saveCardio(data);drawCardio()}
function deleteCardioSession(i){const {data,list}=cardioWeek();list.splice(i,1);saveCardio(data);drawCardio()}

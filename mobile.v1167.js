...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1167.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 let ref=null,laurel=null,footerScene=null,posterRef=null;try{ref=await load('template-reference-v1167.png')}catch(_){ }try{laurel=await load('auth-laurel-realistic-v1167.png')}catch(_){ }try{footerScene=await load('footer-mountains-man-v1167.png')}catch(_){ }try{posterRef=await load('poster-reference-v1167.png')}catch(_){ }
 const atlasFiles={pectoraux:'atlas-pectoraux-v1167.png',dos:'atlas-dos-v1167.png',deltoide_anterieur:'atlas-deltoide_anterieur-v1167.png',deltoide_lateral:'atlas-deltoide_lateral-v1167.png',deltoide_posterieur:'atlas-deltoide_posterieur-v1167.png',trapezes_superieurs:'atlas-trapezes_superieurs-v1167.png',trapezes_moyens:'atlas-trapezes_moyens-v1167.png',cou:'atlas-cou-v1167.png',biceps:'atlas-biceps-v1167.png',triceps:'atlas-triceps-v1167.png',avant_bras:'atlas-avant_bras-v1167.png',abdominaux:'atlas-abdominaux-v1167.png',lombaires:'atlas-lombaires-v1167.png',fessiers:'atlas-fessiers-v1167.png',adducteurs:'atlas-adducteurs-v1167.png',quadriceps:'atlas-quadriceps-v1167.png',ischios:'atlas-ischios-v1167.png',mollets:'atlas-mollets-v1167.png'};
 const atlasImgs={};await Promise.all(Object.entries(atlasFiles).map(async([k,f])=>{try{atlasImgs[k]=await load(f)}catch(_){}}));
 // v1.16.7 — anatomie composite : une seule silhouette + calques musculaires superposables.
 let anatomyBase=null;try{anatomyBase=await load('anatomy-base-v1167.png')}catch(_){ }
 const maskFiles=Object.fromEntries(Object.keys(atlasFiles).map(k=>[k,`mask-${k}-v1167.png`]));
 const muscleMasks={};await Promise.all(Object.entries(maskFiles).map(async([k,f])=>{try{muscleMasks[k]=await load(f)}catch(_){}}));
 ...
 // Header v1.16.7 : zone entièrement reconstruite pour supprimer tout élément fantôme du template.
 ...
 // v1.16.7 — composition éditoriale ForgeLab : pas d'icônes UI, pas de cartes.
 ...
 const anatomy=(cx,cy,names,accent)=>{
   // v1.16.7 : UNE seule anatomie par séance. Tous les muscles sélectionnés
   // sont superposés en or sur exactement la même silhouette face + dos.
   const keys=[...new Set(names.map(atlasKey).filter(Boolean))];
   if(!anatomyBase)return;
   const iw=anatomyBase.naturalWidth||anatomyBase.width, ih=anatomyBase.naturalHeight||anatomyBase.height;
   const maxW=cardW-18,maxH=172,sc=Math.min(maxW/iw,maxH/ih),dw=iw*sc,dh=ih*sc;
   const x=cx-dw/2,y=cy-dh/2;
   g.save();g.imageSmoothingEnabled=true;g.imageSmoothingQuality='high';
   g.drawImage(anatomyBase,x,y,dw,dh);
   keys.forEach(k=>{const im=muscleMasks[k];if(im)g.drawImage(im,x,y,dw,dh)});
   g.restore();
 };
 ...
 // Footer v1.16.7 : scène continue + signature DAILY WINS. dans le ciel.
  ...
  const rows=DAYS.map(day=>{
    ...
    const items=entries.length?entries.map(x=>{
      ...
      return `<div class="mobile-program-entry" data-id="${esc(x.id)}">
        ...
        <div class="mobile-program-muscle"><strong>${esc(x.muscle_name)}</strong><small class="${klass}">${label}</small>${(x.exercises||[]).length?`<small class="program-exercise-summary">${(x.exercises||[]).map(e=>`${esc(e.name)} · ${Number(e.sets||0)}s`).join('  •  ')}</small>`:''}</div>
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1167.png" alt=""><strong>ForgeLab</strong></div>
...
/* ===== ForgeLab v1.16.7 — Programme complet / multi-blocs ===== */

...
function importPcProgrammingV165(){
  ...
  const marker=`forgelabMobileProgrammingImport:v1120:${currentUser.id}`;
...
async function repairSeriesFromCarnet(cloudRows){
  // v1.12.0 : fonction conservée uniquement pour compatibilité interne.
...
function jSeriesBaselineForDraft(id){
  ...
  // Compatibilité immédiate avec les séances déjà validées avant v1.12.0 :
...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1120.png" alt="" aria-hidden="true">
      <div class="auth-brand">FORGELAB</div>
      <div class="auth-sub">ATLAS · ESPACE PERSONNEL</div>
      <div class="auth-version"><span>v1.12.0</span></div>
    </div>
...
function trackingMuscuTrend(done,minimum,inTarget,under,over,totalGroups,overNames=[]){
  ...
  const overDetail=overNames.length
    ? ` Au-dessus du volume prévu : ${overNames.join(', ')}.`
    : '';
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1120.png" alt=""><strong>ForgeLab</strong></div>
    ...
    <div class="story-status-grid">
      <div class="ok"><b>${inTarget}</b><strong>GROUPES MUSCULAIRES TRAVAILLÉS</strong><span>objectif atteint</span></div>
...
// v1.12.0 : aucun nettoyage automatique de Séries.

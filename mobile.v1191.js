...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1191.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 const assets={};
 const files=['anatomy-standard-neutral-v1191.png',...[...standardKeys,...shoulderKeys].map(k=>`mask-${k}-v1191.png`),'auth-laurel-realistic-v1191.png'];
 ...
 const laurel=assets['auth-laurel-realistic-v1191.png'];if(laurel)g.drawImage(laurel,W/2-56,28,112,88);
 ...
 function drawAnatomy(keys,x,y,w,h){
   const base=assets['anatomy-standard-neutral-v1191.png'];if(!base||!keys.length)return;
   const iw=base.naturalWidth||base.width,ih=base.naturalHeight||base.height,sc=Math.min(w/iw,h/ih),dw=iw*sc,dh=ih*sc,dx=x+(w-dw)/2,dy=y+(h-dh)/2;
   g.drawImage(base,dx,dy,dw,dh);
   [...new Set(keys)].forEach(k=>{const m=assets[`mask-${k}-v1191.png`];if(m)g.drawImage(m,dx,dy,dw,dh)});
 }
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1191.png" alt=""><strong>ForgeLab</strong></div>
...
/* ===== ForgeLab v1.19.1 — Brouillon structuré : Programme > Trame > Blocs > Génération ===== */

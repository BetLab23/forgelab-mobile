...
function importPcProgrammingV165(){
  ...
  const marker=`forgelabMobileProgrammingImport:v1113:${currentUser.id}`;
...
async function repairSeriesFromCarnet(cloudRows){
  // v1.11.3 : fonction conservée uniquement pour compatibilité interne.
...
function jSeriesBaselineForDraft(id){
  ...
  // Compatibilité immédiate avec les séances déjà validées avant v1.11.3 :
...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1113.png" alt="" aria-hidden="true">
      <div class="auth-brand">FORGELAB</div>
      <div class="auth-sub">ATLAS · ESPACE PERSONNEL</div>
      <div class="auth-version"><span>v1.11.3</span></div>
    </div>
...
function drawPriorities(){
  ...
  `<section class="priority-overview block-cloud-overview">
    ...
    <p>La durée et le contexte nutritionnel sont définis sur PC et synchronisés avec ce compte.</p>
  </section>`+
...
function trackingMuscuSummary(){
  ...
  const credited=active.reduce((a,p)=>a+Math.min(total(p.muscle_key),(+p.target_min||0)),0);
  const pct=minimum?Math.min(100,Math.round(credited/minimum*100)):0;
  const inTarget=active.filter(p=>{const t=total(p.muscle_key),[mi,ma]=pRange(p);return t>=mi&&t<=ma}).length;
  const under=active.filter(p=>total(p.muscle_key)<pRange(p)[0]).length;
  const over=active.filter(p=>total(p.muscle_key)>pRange(p)[1]).length;
  const todo=active.filter(p=>total(p.muscle_key)<pRange(p)[0]).map(p=>{
    const t=total(p.muscle_key),[mi,ma]=pRange(p),left=Math.max(0,mi-t);
    return `<div class="track-muscu-todo-row ${p.priority.toLowerCase()}"><span>${esc(p.muscle_name)}</span><small>${t} / ${mi}–${ma}</small><b>+${left}</b></div>`;
  }).join('');
  return `<section class="track-muscu-recap">
    <div class="track-recap-title"><div><small>MUSCULATION</small><strong>Semaine ${state.week}</strong></div><span>${trackingWeekDates()}</span></div>
    <div class="track-main-score">
      <div class="track-score-done"><b>${done}</b><span>/ ${minimum}</span><small>séries</small></div>
      <div class="track-score-progress"><b>${pct}%</b><span>Progression vers le minimum</span><div class="track-recap-bar"><i style="width:${pct}%"></i></div></div>
    </div>
    <div class="track-status-grid">
      <div class="ok"><b>${inTarget}</b><span>dans la cible</span></div>
      <div class="low"><b>${under}</b><span>sous la cible</span></div>
      <div class="high"><b>${over}</b><span>au-dessus max</span></div>
    </div>
    <div class="track-remain"><div><small>RESTANT</small><b>${remain}</b><span>séries jusqu'aux minimums</span></div></div>
    ${todo?`<div class="track-muscu-todo"><div class="track-todo-head"><span>À compléter</span><b>${under} groupe${under>1?'s':''}</b></div>${todo}</div>`:`<div class="track-all-done">✓ Tous les minimums sont atteints</div>`}
  </section>`;
}
...
function drawTracking(){
  ...
  `<div class="tracking-date">Semaine ${state.week} · ${trackingWeekDates()}</div>
   ...
   <div id="trackingModeContent">${trackingMode==='muscu'
     ? trackingMuscuSummary()+trackingMuscleDetails()
...
function seriesCleanupKey(){
  return `forgelabCleanup_S3_from_2026_09_15_v1113:${currentUser?.id||'guest'}`;
}
function isAutoDraftAfterCleanupBoundary(d){
  if(!d?.id || !String(d.id).startsWith('auto_'))return false;
  const m=String(d.id).match(/_S(\d+)_J(\d+)$/);
  if(!m)return false;
  const week=+m[1],day=+m[2];
  return week>3 || (week===3 && day>=2);
}
async function cleanupPhantomSeriesFromSep15(){
  if(!currentUser?.id || localStorage.getItem(seriesCleanupKey())==='1')return;

  // S3 = semaine du lundi 14/09/2026.
  // On garde volontairement toutes les séries du lundi (J1).
  // À partir du mardi 15/09 (J2), la semaine est remise à zéro.
  await req(`/forgelab_series?user_id=eq.${encodeURIComponent(uid())}&block_key=eq.main&week=eq.3&day=gte.2`,{
    method:'DELETE',
    headers:{Prefer:'return=minimal'}
  });

  // Les semaines futures ne doivent contenir aucune série réalisée.
  await req(`/forgelab_series?user_id=eq.${encodeURIComponent(uid())}&block_key=eq.main&week=gt.3`,{
    method:'DELETE',
    headers:{Prefer:'return=minimal'}
  });

  // Nettoyage des brouillons fantômes uniquement.
  // Les séances déjà validées dans l'historique ne sont jamais supprimées.
  const before=jDraftLoad();
  const after=before.filter(d=>!isAutoDraftAfterCleanupBoundary(d));
  if(after.length!==before.length){
    jDraftRawSave(after);
    journalCloudMarkDirty();
    try{await pushJournalCloud()}catch(e){console.warn('Nettoyage brouillons cloud différé',e)}
  }

  localStorage.setItem(seriesCleanupKey(),'1');
  console.info(`ForgeLab v1.11.3 : nettoyage S3 effectué (${before.length-after.length} brouillon(s) supprimé(s)).`);
}
...
async function boot(){
  ...
  await cleanupPhantomSeriesFromSep15();
  await load();
  clearInterval(poll);

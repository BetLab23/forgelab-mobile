...
async function generateProgramImage(){
 ...
 const W=1400,H=Math.max(1900,780+activeDays.length*250),c=document.createElement('canvas');c.width=W;c.height=H;const g=c.getContext('2d');
 const bg='#080b0e',panel='#10161b',panel2='#0d1216',gold='#d8ad61',gold2='#f0cf8a',white='#f4f1ea',muted='#a9adb0',blue='#169fe6',green='#14b88a',grey='#879097';
 const rr=(x,y,w,h,r,fill,stroke)=>{g.beginPath();g.roundRect(x,y,w,h,r);if(fill){g.fillStyle=fill;g.fill()}if(stroke){g.strokeStyle=stroke;g.lineWidth=2;g.stroke()}};
 ...
 const fit=(t,x,y,max,size,color=white,weight=700)=>{let z=size;while(z>20){g.font=`${weight} ${z}px Arial`;if(g.measureText(t).width<=max)break;z-=2}txt(t,x,y,z,color,weight)};
 const line=(x1,y1,x2,y2,color='#27323a',w=2)=>{g.beginPath();g.moveTo(x1,y1);g.lineTo(x2,y2);g.strokeStyle=color;g.lineWidth=w;g.stroke()};
 const muscleZone=name=>{const n=(name||'').toLowerCase();if(/quad|isch|adduct|fess|mollet/.test(n))return'legs';if(/pect|delto|épaule|epaule/.test(n))return'chest';if(/dos|trap/.test(n))return'back';if(/biceps|triceps|avant-bras/.test(n))return'arms';if(/abdo/.test(n))return'core';if(/cou/.test(n))return'neck';return'body'};
 const bodyIcon=(cx,cy,names,accent)=>{g.save();g.strokeStyle='#58636b';g.lineWidth=6;g.lineCap='round';g.beginPath();g.arc(cx,cy-46,16,0,Math.PI*2);g.stroke();g.beginPath();g.moveTo(cx,cy-28);g.lineTo(cx,cy+42);g.moveTo(cx-34,cy-8);g.lineTo(cx+34,cy-8);g.moveTo(cx-30,cy-5);g.lineTo(cx-48,cy+35);g.moveTo(cx+30,cy-5);g.lineTo(cx+48,cy+35);g.moveTo(cx,cy+42);g.lineTo(cx-25,cy+88);g.moveTo(cx,cy+42);g.lineTo(cx+25,cy+88);g.stroke();const zones=new Set(names.map(muscleZone));g.fillStyle=accent;g.globalAlpha=.95;if(zones.has('neck'))g.fillRect(cx-9,cy-30,18,14);if(zones.has('chest')){g.beginPath();g.ellipse(cx,cy-5,29,18,0,0,Math.PI*2);g.fill()}if(zones.has('back')){g.beginPath();g.ellipse(cx,cy+10,25,27,0,0,Math.PI*2);g.fill()}if(zones.has('arms')){g.fillRect(cx-45,cy-14,18,48);g.fillRect(cx+27,cy-14,18,48)}if(zones.has('core'))g.fillRect(cx-15,cy+12,30,30);if(zones.has('legs')){g.fillRect(cx-27,cy+42,18,43);g.fillRect(cx+9,cy+42,18,43)}g.restore()};
 // Background + premium framing
 g.fillStyle=bg;g.fillRect(0,0,W,H);const grd=g.createLinearGradient(0,0,W,H);grd.addColorStop(0,'rgba(216,173,97,.08)');grd.addColorStop(.5,'rgba(0,0,0,0)');grd.addColorStop(1,'rgba(22,159,230,.05)');g.fillStyle=grd;g.fillRect(0,0,W,H);line(55,54,W-55,54,gold,3);
 // Laurel ForgeLab logo
 let logo=null;try{logo=await new Promise((res,rej)=>{const im=new Image();im.onload=()=>res(im);im.onerror=rej;im.src='auth-laurel-realistic-v1127.png'})}catch(_){ }
 if(logo)g.drawImage(logo,62,78,130,130);else{txt('❧',125,140,86,gold,700,'center')}
 txt('FORGELAB',220,118,52,gold2,700);txt('DISCIPLINA · CONSTANTIA · PROGRESSUS',222,160,18,gold,500);txt('TRAIN  ·  TRACK  ·  PROGRESS',W-70,105,18,muted,500,'right');
 // Title
 fit((meta.name||'BLOC FORGELAB').toUpperCase(),70,270,W-140,64,gold2,800);txt('PROGRAMMATION · PLAN DE BLOC',72,325,23,white,500);line(70,365,W-70,365,'#5b482b',2);
 // Metadata cards
 const cards=[['DURÉE',`${meta.duration} SEMAINES`],['OBJECTIF','HYPERTROPHIE'],['FRÉQUENCE',`${activeDays.length} SÉANCES / SEMAINE`],['TRAME',`${meta.rhythm} + DELOAD`]];const cw=(W-140-45)/4;cards.forEach((a,i)=>{const x=70+i*(cw+15);rr(x,395,cw,112,12,panel,'#5c4a2f');txt(a[0],x+22,425,17,muted,600);fit(a[1],x+22,470,cw-44,27,white,800)});
 // Cycles
 txt('CYCLES',70,565,30,gold2,800);const gap=12,pw=(W-140-gap*(ph.length-1))/ph.length;ph.forEach((q,i)=>{const x=70+i*(pw+gap),ac=q.deload?grey:(i%2?green:blue);rr(x,610,pw,170,10,panel2,ac);g.fillStyle=ac;g.fillRect(x,610,pw,42);txt(q.deload?'DELOAD':q.label.toUpperCase(),x+pw/2,632,19,q.deload?'#17191b':'#061016',800,'center');txt(`S${q.from}${q.to!==q.from?' – S'+q.to:''}`,x+pw/2,690,26,white,800,'center');txt(q.deload?'RÉCUPÉRATION':'DÉVELOPPEMENT',x+pw/2,735,16,q.deload?muted:ac,700,'center')});
 // Week layout
 let y=845;txt(`TRAME HEBDOMADAIRE (${activeDays.length} SÉANCE${activeDays.length>1?'S':''})`,70,y,30,gold2,800);y+=48;
 activeDays.forEach((d,idx)=>{const entries=(p[d]||[]).filter(z=>+z.sets>0),accent=idx%2?green:blue,total=entries.reduce((a,z)=>a+(+z.sets||0),0);rr(70,y,W-140,215,14,panel,'#26343d');g.fillStyle=accent;g.fillRect(70,y,190,215);txt(d.toUpperCase(),165,y+35,26,'#071015',900,'center');bodyIcon(165,y+105,entries.map(z=>z.muscle_name), '#071015');txt(`${total} SÉRIES`,165,y+188,18,'#071015',800,'center');
   const left=300, colW=(W-left-90)/2;entries.forEach((z,j)=>{const col=j%2,row=Math.floor(j/2),xx=left+col*colW,yy=y+45+row*43;txt(z.muscle_name,xx,yy,21,white,650);txt(`${z.sets} séries`,xx+colW-35,yy,19,accent,800,'right')});
   y+=235;
 });
 // Footer
 y+=18;rr(70,y,W-140,170,14,'#0b1014','#5c4a2f');txt('DAILY WINS.',W/2,y+58,42,gold2,700,'center','Georgia');line(W/2-85,y+92,W/2+85,y+92,gold,3);txt('La régularité d’aujourd’hui crée la liberté de demain.',W/2,y+128,22,white,500,'center','Georgia');
 txt('FORGELAB',70,H-55,24,gold2,800);txt('DISCIPLINA · CONSTANTIA · PROGRESSUS',W-70,H-55,16,muted,500,'right');

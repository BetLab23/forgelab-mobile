...
function createProgrammingDraft(copyActive=true){const src=copyActive?programmingDataFromKey(programmingKey()):blankProgramming();localStorage.setItem(programmingDraftKey(),JSON.stringify(src));localStorage.setItem(programmingModeKey(),'draft');drawProgramming()}
function activateProgrammingDraft(){if(!hasProgrammingDraft())return;if(!confirm('Activer ce brouillon ? Le programme actif sera remplacé.'))return;localStorage.setItem(programmingKey(),localStorage.getItem(programmingDraftKey()));localStorage.removeItem(programmingDraftKey());localStorage.setItem(programmingModeKey(),'active');drawProgramming()}
...
function drawProgramming(){
  ...
  const mode=programmingMode(),hasDraft=hasProgrammingDraft();
  v.innerHTML=title('Programmation des séances')+
   `<section class="program-workspace"><div class="program-workspace-tabs"><button type="button" data-program-mode="active" class="${mode==='active'?'active':''}">Programme actif</button><button type="button" data-program-mode="draft" class="${mode==='draft'?'active':''}" ${hasDraft?'':'disabled'}>Brouillon</button></div>${mode==='active'?`<p>Ta programmation utilisée actuellement reste protégée.</p><button type="button" data-program-create-draft>${hasDraft?'Ouvrir le brouillon':'Créer un brouillon'}</button>`:`<p>Tu peux préparer ta prochaine programmation sans modifier l’actuelle.</p><div class="program-workspace-actions"><button type="button" data-program-activate-draft>Activer ce programme</button><button type="button" class="danger" data-program-delete-draft>Supprimer le brouillon</button></div>`}</section>`+

...
async function repairSeriesFromCarnet(cloudRows){
  // v1.21.9 : fonction conservée uniquement pour compatibilité interne.
...
function jSeriesBaselineForDraft(id){
  ...
  // Compatibilité immédiate avec les séances déjà validées avant v1.21.9 :
...
function jExportPdf(sessions,from='',to=''){
 const rows=[...(sessions||[])].sort((a,b)=>String(a.date||'').localeCompare(String(b.date||''))||jHistoryTime(a)-jHistoryTime(b));
 if(!rows.length){alert('Aucune séance à exporter.');return}
 const first=from||rows[0]?.date||'',last=to||rows.at(-1)?.date||'';
 const totalSets=rows.reduce((n,s)=>n+jSets(s),0),totalMin=rows.reduce((n,s)=>n+(+s.duration||0),0),totalTon=rows.reduce((n,s)=>n+jTon(s),0);
 const detail=rows.map(s=>`<section class="session"><div class="sessionHead"><div><small>${jDate(s.date)}</small><h2>${esc(s.sessionName||'Séance sans nom')}</h2></div><div class="sessionStats"><b>${s.duration||0}<i>min</i></b><b>${jSets(s)}<i>séries</i></b><b>${jFmt(jTon(s))}<i>kg</i></b></div></div>${(s.exercises||[]).map(e=>`<div class="exercise"><strong>${esc(e.muscle||'')} · ${esc(e.name||'Exercice')}</strong><span>${(e.sets||[]).map(x=>`${jFmt(x.kg)} kg × ${+x.reps||0}`).join(' · ')}</span></div>`).join('')}${s.notes?`<div class="notes"><b>Observations</b>${esc(s.notes)}</div>`:''}</section>`).join('');
 const html=`<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>ForgeLab · Carnet</title><style>@page{size:A4;margin:12mm}*{box-sizing:border-box}body{margin:0;background:#0d0b08;color:#eee4d2;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif}main{max-width:900px;margin:auto;padding:28px}.brand{text-align:center;border-bottom:1px solid #8c6a35;padding-bottom:18px;margin-bottom:20px}.laurel{font:700 34px Georgia,serif;color:#d9b66f;letter-spacing:.06em}.brand h1{margin:3px 0 0;font:700 27px Georgia,serif;letter-spacing:.16em;color:#f1d59b}.brand p{margin:5px 0 0;font-size:9px;letter-spacing:.2em;color:#a98a57}.period{text-align:center;color:#bda779;font-size:12px;margin-top:10px}.summary{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:18px 0}.summary div,.session{border:1px solid #5b472b;background:#17120c;border-radius:12px}.summary div{text-align:center;padding:12px 6px}.summary b{display:block;color:#edcf91;font-size:20px}.summary small{font-size:8px;text-transform:uppercase;color:#968b79}.session{padding:14px;margin:0 0 12px;break-inside:avoid}.sessionHead{display:flex;justify-content:space-between;gap:14px;align-items:center}.sessionHead small{color:#b99050}.session h2{margin:3px 0 8px;font:700 18px Georgia,serif}.sessionStats{display:flex;gap:12px;text-align:center}.sessionStats b{color:#e5c37f}.sessionStats i{display:block;color:#8f8678;font:8px sans-serif;text-transform:uppercase}.exercise{display:flex;justify-content:space-between;gap:15px;padding:8px 0;border-top:1px solid #2f281e;font-size:10px}.exercise span{text-align:right;color:#c9b58d}.notes{margin-top:8px;padding:9px;background:#100d09;border-left:2px solid #a87d3f;font-size:10px;white-space:pre-wrap}.notes b{display:block;color:#d8b775;margin-bottom:3px}.foot{text-align:center;color:#8d7752;font-size:9px;letter-spacing:.16em;margin-top:20px}@media print{body{background:#0d0b08!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}main{padding:0}.session{break-inside:avoid}}</style></head><body><main><header class="brand"><div class="laurel">❧ FORGELAB ❧</div><h1>CARNET D'ENTRAÎNEMENT</h1><p>DISCIPLINA · CONSTANTIA · PROGRESSUS</p><div class="period">${jDate(first)} → ${jDate(last)}</div></header><div class="summary"><div><b>${rows.length}</b><small>Séances</small></div><div><b>${totalSets}</b><small>Séries</small></div><div><b>${totalMin}</b><small>Minutes</small></div><div><b>${jFmt(totalTon)}</b><small>kg tonnage</small></div></div>${detail}<div class="foot">DAILY WINS. · FORGELAB</div></main><script>setTimeout(()=>window.print(),350)<\/script></body></html>`;
 const w=window.open('','_blank');
 if(!w){alert("Safari a bloqué l'export. Autorise temporairement les fenêtres surgissantes puis réessaie.");return}
 w.document.open();w.document.write(html);w.document.close();
}
async function jClearSavedSessions(){
 const count=jLoad().length;
 if(!count)return;
 const ok=confirm(`Vider le carnet ?\n\n${count} séance${count>1?'s seront supprimées':' sera supprimée'} du Carnet.\n\nLes brouillons, Séries, Suivi, Programmation et Priorités restent conservés. Cette action est définitive.`);
 if(!ok)return;
 const second=confirm(`Confirmation finale : supprimer définitivement ${count} séance${count>1?'s':''} enregistrée${count>1?'s':''} ?`);
 if(!second)return;
 jSave([]);
 try{await pushJournalCloud()}catch(_){}
...
function drawJournal(){
 ...
 <section class="j-card j-manage"><div class="j-section">Gestion du carnet <span>${all.length} séance${all.length>1?'s':''}</span></div><p class="j-manage-help">Exporte tes séances enregistrées avant d'archiver ton carnet. Les brouillons, Séries, Suivi, Programmation et Priorités ne sont pas supprimés.</p><div class="j-range"><label>Du<input id="jExportFrom" type="date" value="${all.length?[...all].sort((a,b)=>String(a.date||'').localeCompare(String(b.date||'')))[0]?.date||'':''}"></label><label>Au<input id="jExportTo" type="date" value="${all.length?[...all].sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')))[0]?.date||'':''}"></label></div><button id="jExportAll" class="j-wide" ${all.length?'':'disabled'}>Exporter tout le carnet en PDF</button><button id="jExportRange" class="j-wide j-secondary" ${all.length?'':'disabled'}>Exporter la période sélectionnée</button><button id="jClear" class="j-clear" ${all.length?'':'disabled'}>Vider les séances enregistrées</button></section>`;
 ...
 const exportAll=document.querySelector('#jExportAll'),exportRange=document.querySelector('#jExportRange'),clearBtn=document.querySelector('#jClear');
 if(exportAll)exportAll.onclick=()=>jExportPdf(all);
 if(exportRange)exportRange.onclick=()=>{const from=document.querySelector('#jExportFrom')?.value||'',to=document.querySelector('#jExportTo')?.value||'';const picked=all.filter(s=>(!from||String(s.date||'')>=from)&&(!to||String(s.date||'')<=to));if(!picked.length){alert('Aucune séance enregistrée sur cette période.');return}jExportPdf(picked,from,to)};
 if(clearBtn)clearBtn.onclick=()=>jClearSavedSessions();
}
...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1223.png" alt="" aria-hidden="true">
      <div class="auth-brand">FORGELAB</div>
      <div class="auth-sub">ATLAS · ESPACE PERSONNEL</div>
      <div class="auth-version"><span>v1.21.9</span></div>
    </div>
...
async function generateProgramImage(){
 ...
 const assets={};for(const f of ['auth-laurel-realistic-v1223.png','footer-mountains-man-v1223.png']){try{assets[f]=await load(f)}catch(_){}}
 ...
 const laurel=assets['auth-laurel-realistic-v1223.png'];
 ...
 const footer=assets['footer-mountains-man-v1223.png'];if(footer){g.save();g.globalAlpha=.66;const fw=W,fh=150,sy=Math.max(0,footer.height-footer.width*(fh/fw));g.drawImage(footer,0,sy,footer.width,footer.height-sy,0,H-footerH,fw,footerH);g.restore()}
...
function flBlockPeriod(block,fp=flActiveProgram()){
  ...
  // Compatibilité exceptionnelle pour l'ancien programme actif.
  // IMPORTANT : ces anciens programmes n'avaient pas de structure de blocs fiable.
  // La migration pouvait donc transformer le bloc historique en simple S1.
  // Pour le bilan, on reconstruit la plage de semaines depuis la date choisie
  // jusqu'à la semaine réellement atteinte aujourd'hui, sans modifier les données.
  if(block.exportStartDate&&/^\d{4}-\d{2}-\d{2}$/.test(block.exportStartDate)){
    const [y,m,d]=block.exportStartDate.split('-').map(Number),start=new Date(y,m-1,d,12);
    ...
    const lastWeek=Math.max(1,Math.floor(elapsed/7)+1);
    // Ne jamais se limiter au from/to artificiel du bloc migré. Les séries historiques
    // sont stockées avec leurs vrais numéros de semaine (S1, S2, S3...).
    const availableWeeks=(state.series||[]).map(r=>+r.week||0).filter(w=>w>0);
    const maxDataWeek=availableWeeks.length?Math.max(...availableWeeks):1;
    const exportTo=Math.max(lastWeek,maxDataWeek);
    const end=new Date(start);end.setDate(end.getDate()+exportTo*7-1);
    return {start,end,from:1,to:exportTo,label:`${flPdfDate(start)} – ${flPdfDate(end)}`,legacyOverride:true};
  }
  return null;
}
...
function exportBlockPdf(blockId){
  ...
  const priorities=activePriorities(), muscleNames=new Map((state.priorities||[]).map(x=>[x.muscle_key,x.muscle_name||x.muscle_key]));
  const allRows=(state.series||[]).filter(r=>+r.week>=period.from&&+r.week<=period.to&&(+r.series||0)>0);
  const cardio=cardioData(), ymd=d=>`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
  const allCardioSessions=(cardio.sessions||[]).filter(x=>x&&!x.benchmark&&x.date&&x.date>=ymd(period.start)&&x.date<=ymd(period.end));
  const weekDates=wk=>{const a=new Date(period.start);a.setDate(a.getDate()+(wk-period.from)*7);const b=new Date(a);b.setDate(b.getDate()+6);return {a,b,label:`${flPdfDate(a)} – ${flPdfDate(b)}`}};
  const weekData=[];
  for(let wk=period.from;wk<=period.to;wk++){
    const wr=allRows.filter(r=>+r.week===wk), byKey={};wr.forEach(r=>{const k=r.muscle_key||r.muscle_name||'Groupe';byKey[k]=(byKey[k]||0)+(+r.series||0)});
    const min=priorities.reduce((a,p)=>a+(+p.target_min||0),0), done=priorities.reduce((a,p)=>a+(byKey[p.muscle_key]||0),0);
    const under=priorities.filter(p=>(byKey[p.muscle_key]||0)<(+p.target_min||0)), over=priorities.filter(p=>(byKey[p.muscle_key]||0)>(+p.target_max||0));
    const reached=priorities.length-under.length, remain=priorities.reduce((a,p)=>a+Math.max(0,(+p.target_min||0)-(byKey[p.muscle_key]||0)),0), pct=min?Math.round(done/min*100):0;
    const sessions=new Set(wr.map(r=>r.day).filter(Boolean)).size, wd=weekDates(wk);
    const cw=cardio.weeks?.[String(wk)]||{}, cardioDone=Object.values(cw).reduce((a,days)=>a+Object.values(days||{}).reduce((x,n)=>x+(+n||0),0),0);
    const cardioTarget=(cardio.modalities||[]).reduce((a,m)=>a+(+m.target||0),0);
    const runs=allCardioSessions.filter(x=>x.date>=ymd(wd.a)&&x.date<=ymd(wd.b));
    let trend;if(under.length===0&&over.length===0)trend=`Semaine complète : ${pct}% du volume minimum réalisé et ${reached}/${priorities.length} objectifs musculaires atteints.`;
    else if(under.length===0)trend=`Semaine complète : ${reached}/${priorities.length} objectifs musculaires atteints.${over.length?` ${over.length} groupe${over.length>1?'s':''} avec volume dépassé (${over.map(p=>p.muscle_name||p.muscle_key).join(', ')}).`:''}`;
    else if(pct>=90)trend=`Semaine presque complète : ${pct}% du volume minimum réalisé. Il reste ${under.length} groupe${under.length>1?'s':''} à compléter.`;
    else if(pct>=70)trend=`Bonne progression : ${pct}% du volume minimum réalisé. ${under.length} groupe${under.length>1?'s restent':' reste'} à compléter.`;
    else trend=`Semaine en cours : ${pct}% du volume minimum réalisé. Priorité aux ${under.length} groupe${under.length>1?'s':''} encore sous leur cible.`;
    weekData.push({wk,wr,byKey,min,done,under,over,reached,remain,pct,sessions,wd,cardioDone,cardioTarget,runs,trend});
  }
  const performed={};allRows.forEach(r=>{const name=r.muscle_name||muscleNames.get(r.muscle_key)||r.muscle_key||'Groupe';performed[name]=(performed[name]||0)+(+r.series||0)});
  const muscleTotal=Object.values(performed).reduce((a,n)=>a+n,0),muscleSessions=new Set(allRows.map(r=>`${r.week}:${r.day}`).filter(Boolean)).size;
  const cardioTotal=weekData.reduce((a,w)=>a+w.cardioDone,0), cardioCount=Math.max(cardioTotal,allCardioSessions.length), notes=flActiveBlockNotes(block.id), maxMuscle=Math.max(1,...Object.values(performed));
  const muscleBars=Object.entries(performed).sort((a,b)=>b[1]-a[1]).map(([k,v])=>`<div class="muscle-bar"><span>${esc(k)}</span><div><i style="width:${Math.max(4,Math.round(v/maxMuscle*100))}%"></i></div><b>${v}</b></div>`).join('');
  const weeklyCards=weekData.map(w=>{const status=w.under.length?'À compléter':w.over.length?'Volume dépassé':'Validée';const cls=w.under.length?'warn':w.over.length?'high':'ok';const detail=priorities.map(p=>{const n=w.byKey[p.muscle_key]||0,mi=+p.target_min||0,ma=+p.target_max||0,c=n<mi?'low':n>ma?'high':'ok';return `<div class="mini-muscle ${c}"><span>${esc(p.muscle_name||p.muscle_key)}</span><b>${n}</b><small>${mi}–${ma}</small></div>`}).join('');const run=w.runs[0];return `<section class="week-card"><div class="week-head"><div><small>BILAN HEBDOMADAIRE</small><h2>S${w.wk}</h2><span>${w.wd.label}</span></div><em class="${cls}">${status}</em></div><div class="week-kpis"><div><b>${w.done}</b><span>/ ${w.min} séries</span></div><div><b>${w.pct}%</b><span>du minimum</span></div><div><b>${w.sessions}</b><span>séance${w.sessions>1?'s':''} muscu</span></div><div><b>${w.reached}</b><span>/ ${priorities.length} groupes OK</span></div></div><div class="week-progress"><i style="width:${Math.min(100,w.pct)}%"></i></div><div class="status-row"><span class="ok">${w.reached} travaillés</span><span class="warn">${w.under.length} à compléter</span><span class="high">${w.over.length} dépassés</span><span>${w.remain} séries restantes</span></div><div class="trend"><small>TENDANCE DE LA SEMAINE</small><p>${esc(w.trend)}</p></div><details open><summary>Détail musculaire</summary><div class="mini-grid">${detail}</div></details><div class="cardio-week"><strong>CARDIO</strong><span>${w.cardioDone} / ${w.cardioTarget||0} séance${w.cardioDone>1?'s':''}</span>${run?`<small>${run.duration||'—'} min · ${esc(run.pace||'—')}/km · ${run.avgHr||'—'} bpm · RPE ${run.rpe||'—'}</small>`:'<small>Aucune donnée détaillée enregistrée</small>'}</div></section>`}).join('');
  const pdfLaurelUrl='data:image/png;base64,';
  const html=`<!doctype html><html><head><meta charset="utf-8"><title>ForgeLab · Bilan ${esc(block.name||'Bloc')}</title><style>
  @page{size:A4;margin:10mm}*{box-sizing:border-box}body{margin:0;background:#070809;color:#f0f0ee;font-family:Arial,Helvetica,sans-serif;-webkit-print-color-adjust:exact;print-color-adjust:exact}.sheet{background:radial-gradient(circle at 50% 0,#242016 0,#111315 28%,#070809 72%);padding:22px;border:1px solid #7c6232;min-height:277mm}.cover{page-break-after:always}.brand{text-align:center;padding:8px 0 18px;border-bottom:1px solid #6e582f}.laurel{height:62px;display:flex;align-items:center;justify-content:center;margin:0 auto 5px}.laurel img{display:block;max-height:62px;max-width:118px;object-fit:contain;filter:drop-shadow(0 2px 7px rgba(0,0,0,.65))}.brand-name{font-size:25px;font-weight:900;letter-spacing:6px;color:#e4c47b}.motto{font-size:8px;color:#8e8778;letter-spacing:2px;margin-top:5px}.hero{padding:26px 0 18px}.hero small{color:#9b8a65;letter-spacing:2px}.hero h1{margin:5px 0;font-size:30px;text-transform:uppercase}.hero p{margin:0;color:#aaa}.date-pill{display:inline-block;margin-top:12px;border:1px solid #8d7039;padding:7px 11px;color:#e5c982;font-size:11px}.kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:10px 0 22px}.kpi{background:linear-gradient(145deg,#17191b,#0d0e0f);border:1px solid #443a27;padding:12px}.kpi b{display:block;color:#e4c47b;font-size:24px}.kpi span{font-size:8px;color:#999;text-transform:uppercase;letter-spacing:.8px}.section-title{margin:20px 0 10px;font-size:11px;letter-spacing:2px;color:#d8b86d;border-bottom:1px solid #4b402b;padding-bottom:7px}.muscle-bar{display:grid;grid-template-columns:125px 1fr 28px;gap:8px;align-items:center;margin:7px 0;font-size:9px}.muscle-bar>div{height:7px;background:#242424;border-radius:6px;overflow:hidden}.muscle-bar i{display:block;height:100%;background:linear-gradient(90deg,#8b6a2d,#e3c06b);border-radius:6px}.muscle-bar b{text-align:right;color:#e6ca86}.notes{white-space:pre-wrap;background:#111315;border-left:3px solid #b58e46;padding:12px;font-size:10px;line-height:1.5}.week-card{page-break-inside:avoid;margin:0 0 14px;padding:17px;background:linear-gradient(145deg,#151719,#0a0b0c);border:1px solid #5b4929;box-shadow:inset 0 0 0 1px #1f2020}.week-head{display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #3d3526;padding-bottom:10px}.week-head small{font-size:7px;color:#8d816a;letter-spacing:1.5px}.week-head h2{display:inline-block;margin:2px 10px 0 0;color:#e4c47b;font-size:24px}.week-head span{font-size:9px;color:#aaa}.week-head em{font-style:normal;font-size:8px;text-transform:uppercase;padding:5px 8px;border:1px solid}.ok{color:#9fc59f}.warn{color:#e3bd6c}.high{color:#e58d78}.week-kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin:12px 0 7px}.week-kpis>div{background:#0c0d0e;padding:8px;border:1px solid #292823}.week-kpis b{display:block;font-size:16px;color:#e6c77e}.week-kpis span{font-size:7px;color:#8f8f8f}.week-progress{height:5px;background:#272727}.week-progress i{display:block;height:100%;background:linear-gradient(90deg,#8c6b2f,#e4c36f)}.status-row{display:flex;gap:12px;flex-wrap:wrap;margin:8px 0;font-size:8px}.trend{background:#0d0f10;border-left:2px solid #b38c46;padding:9px;margin:9px 0}.trend small{font-size:7px;color:#b49b69;letter-spacing:1px}.trend p{margin:4px 0 0;font-size:9px;line-height:1.4}.week-card details summary{font-size:8px;color:#a99670;margin:8px 0}.mini-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:4px}.mini-muscle{display:grid;grid-template-columns:1fr auto;gap:2px;background:#0b0c0d;border:1px solid #282724;padding:5px;font-size:7px}.mini-muscle b{color:#e5c57b}.mini-muscle small{grid-column:1/-1;color:#777}.mini-muscle.low{border-color:#5e4e2d}.mini-muscle.high{border-color:#633d34}.cardio-week{margin-top:10px;padding-top:8px;border-top:1px solid #343027;display:grid;grid-template-columns:80px 1fr;gap:3px;font-size:8px}.cardio-week strong{color:#d9b86d}.cardio-week small{grid-column:2;color:#8d8d8d}.footer{text-align:center;color:#b8924b;letter-spacing:3px;font-size:8px;margin-top:18px}@media print{body{background:#070809}.sheet{min-height:auto}}
  </style></head><body><main class="sheet cover"><header class="brand"><div class="laurel"><img src="${pdfLaurelUrl}" alt="Couronne de laurier ForgeLab"></div><div class="brand-name">FORGELAB</div><div class="motto">DISCIPLINA · CONSTANTIA · PROGRESSUS</div></header><section class="hero"><small>RAPPORT DE FIN DE BLOC</small><h1>${esc(block.name||'Bloc')}</h1><p>${esc(fp.name||'Programme ForgeLab')} · S${period.from}${period.to!==period.from?' à S'+period.to:''}</p><div class="date-pill">${period.label}</div></section><section class="kpis"><div class="kpi"><b>${muscleTotal}</b><span>Séries réalisées</span></div><div class="kpi"><b>${muscleSessions}</b><span>Séances muscu</span></div><div class="kpi"><b>${cardioCount}</b><span>Séances cardio</span></div><div class="kpi"><b>${weekData.length}</b><span>Semaines analysées</span></div></section><div class="section-title">VOLUME MUSCULAIRE · BLOC COMPLET</div>${muscleBars||'<p>Aucune série renseignée.</p>'}${notes?`<div class="section-title">NOTES DU BLOC</div><div class="notes">${esc(notes)}</div>`:''}<div class="footer">DAILY WINS.</div></main><main class="sheet"><div class="section-title">COMPTES RENDUS HEBDOMADAIRES · SOURCE : SUIVI</div>${weeklyCards}<div class="footer">FORGELAB · DAILY WINS.</div></main><script>const flPrint=()=>setTimeout(()=>{try{window.focus();window.print()}catch(e){}},350);if(document.readyState==='complete')flPrint();else{window.addEventListener('load',flPrint,{once:true});setTimeout(flPrint,1200)}<\/script></body></html>`;
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1223.png" alt=""><strong>ForgeLab</strong></div>
...
// v1.21.9 : aucun nettoyage automatique de Séries.
...
/* ===== ForgeLab v1.21.9 — Brouillon structuré : Programme > Trame > Blocs > Génération ===== */
...
function activateFullProgrammingDraft(){
  ...
  // v1.21.9 : conserver aussi les notes et la trame cardio propres à chaque bloc lors de l'activation.
  fp.blocks.forEach(b=>{
...
drawProgramming=function(){
  ...
  const mode=programmingMode();if(mode!=='draft'){
    ...
    // v1.21.9 : le programme actif reprend la même architecture par blocs que le brouillon.
...
/* ===== ForgeLab v1.21.9 — Cardio du bloc : trame uniquement, séance dans Séries ===== */
...
/* ===== ForgeLab v1.21.9 — Focus bloc + notes libres ===== */

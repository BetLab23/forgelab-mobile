...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1130.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 let ref=null,laurel=null;try{ref=await load('template-reference-v1130.png')}catch(_){ }try{laurel=await load('auth-laurel-realistic-v1130.png')}catch(_){ }
 ...
 box(0,0,470,94,'rgba(2,7,11,.99)');if(laurel)g.drawImage(laurel,22,10,82,82);txt('FORGELAB',112,39,34,white,900);txt('TRAIN  TRACK  PROGRESS',114,72,12,white,500);line(0,94,W,94,blue,2);
 ...
 const anatomy=(cx,cy,names,accent)=>{const Z=new Set(names.map(zone));g.save();g.translate(cx,cy);g.fillStyle='#26343c';g.strokeStyle='#52616a';g.lineWidth=1.3;g.beginPath();g.arc(0,-43,11,0,Math.PI*2);g.fill();g.stroke();g.beginPath();g.moveTo(-8,-30);g.lineTo(-23,-17);g.lineTo(-19,30);g.lineTo(-9,52);g.lineTo(0,58);g.lineTo(9,52);g.lineTo(19,30);g.lineTo(23,-17);g.lineTo(8,-30);g.closePath();g.fill();g.stroke();g.beginPath();g.moveTo(-21,-16);g.lineTo(-34,0);g.lineTo(-37,35);g.lineTo(-29,37);g.lineTo(-20,5);g.closePath();g.moveTo(21,-16);g.lineTo(34,0);g.lineTo(37,35);g.lineTo(29,37);g.lineTo(20,5);g.closePath();g.fill();g.stroke();g.beginPath();g.moveTo(-9,51);g.lineTo(-19,75);g.lineTo(-16,112);g.lineTo(-7,112);g.lineTo(-1,61);g.closePath();g.moveTo(9,51);g.lineTo(19,75);g.lineTo(16,112);g.lineTo(7,112);g.lineTo(1,61);g.closePath();g.fill();g.stroke();g.fillStyle=accent;if(Z.has('neck'))g.fillRect(-6,-31,12,10);if(Z.has('chest')){g.beginPath();g.ellipse(-8,-8,11,9,0,0,Math.PI*2);g.ellipse(8,-8,11,9,0,0,Math.PI*2);g.fill()}if(Z.has('shoulder')){g.beginPath();g.arc(-19,-15,8,0,Math.PI*2);g.arc(19,-15,8,0,Math.PI*2);g.fill()}if(Z.has('back')){g.beginPath();g.moveTo(-18,-12);g.lineTo(-10,26);g.lineTo(0,40);g.lineTo(10,26);g.lineTo(18,-12);g.lineTo(0,-2);g.closePath();g.fill()}if(Z.has('arms')){g.fillRect(-34,-8,8,37);g.fillRect(26,-8,8,37)}if(Z.has('core'))g.fillRect(-8,5,16,29);if(Z.has('legs')){g.fillRect(-17,62,10,45);g.fillRect(7,62,10,45)}g.restore()};
 activeDays.slice(0,6).forEach((d,i)=>{const x=26+i*(cardW+cardGap),entries=(p[d]||[]).filter(z=>+z.sets>0),ac=i%2?green:blue;g.fillStyle='rgba(6,13,18,.98)';g.fillRect(x,cardY,cardW,cardH);g.strokeStyle=ac;g.strokeRect(x,cardY,cardW,cardH);g.fillStyle=ac;g.globalAlpha=.25;g.fillRect(x,cardY,cardW,38);g.globalAlpha=1;txt(d.toUpperCase(),x+cardW/2,cardY+20,14,white,900,'center');fit(entries.slice(0,3).map(z=>z.muscle_name).join(' · '),x+cardW/2,cardY+60,cardW-14,11,white,700,'center');anatomy(x+cardW/2,cardY+145,entries.map(z=>z.muscle_name),ac);let yy=cardY+270;entries.slice(0,6).forEach(z=>{fit(z.muscle_name,x+8,yy,cardW-60,11,white,600);txt(`${z.sets}`,x+cardW-10,yy,11,white,800,'right');yy+=24});g.fillStyle='#101a20';g.fillRect(x+8,cardY+cardH-47,cardW-16,37);txt('FOCUS',x+14,cardY+cardH-35,10,ac,900);fit(entries.slice(0,3).map(z=>z.muscle_name).join(' / '),x+14,cardY+cardH-19,cardW-28,9,white,500)});
 ...
 // Keep the cinematic mountain area from the reference, but clean its old copy and add the validated wording.
 box(315,1240,685,155,'rgba(3,8,12,.55)');txt('PLUS QU’UN PROGRAMME',350,1292,18,white,900);txt('UN PROCESSUS.',350,1320,18,white,900);line(350,1347,430,1347,blue,4);txt('DAILY WINS.',720,1306,28,white,700,'center','Georgia');txt('« La régularité d’aujourd’hui crée la liberté de demain. »',720,1350,14,white,500,'center','Georgia');
 box(0,1438,1024,98,'rgba(2,7,11,.98)');if(laurel)g.drawImage(laurel,25,1450,62,62);txt('FORGELAB',95,1472,23,white,900);txt('DISCIPLINA · CONSTANTIA · PROGRESSUS',95,1501,9,muted,500);txt('GÉNÉRÉ PAR FORGELAB',990,1490,9,muted,500,'right');
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1130.png" alt=""><strong>ForgeLab</strong></div>

...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1189.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 const assets={};
 const files=['anatomy-standard-neutral-v1189.png','anatomy-shoulder-neutral-v1189.png',...standardKeys.map(k=>`mask-${k}-v1189.png`),...shoulderKeys.map(k=>`mask-${k}-v1189.png`),'auth-laurel-realistic-v1189.png'];
 ...
 const laurel=assets['auth-laurel-realistic-v1189.png'];if(laurel)g.drawImage(laurel,W/2-56,28,112,88);
 ...
 function drawAnatomy(keys,x,y,w,h){
   const std=keys.filter(k=>standardKeys.includes(k)), sh=keys.filter(k=>shoulderKeys.includes(k));
   const sets=[];if(std.length)sets.push({base:'anatomy-standard-neutral-v1189.png',keys:std});if(sh.length)sets.push({base:'anatomy-shoulder-neutral-v1189.png',keys:sh});
   if(!sets.length)return;const eachW=w/sets.length;
   sets.forEach((set,i)=>{const base=assets[set.base];if(!base)return;const iw=base.naturalWidth||base.width,ih=base.naturalHeight||base.height,sc=Math.min((eachW-8)/iw,h/ih),dw=iw*sc,dh=ih*sc,dx=x+i*eachW+(eachW-dw)/2,dy=y+(h-dh)/2;g.drawImage(base,dx,dy,dw,dh);set.keys.forEach(k=>{const m=assets[`mask-${k}-v1189.png`];if(m)g.drawImage(m,dx,dy,dw,dh)})});
 }
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1189.png" alt=""><strong>ForgeLab</strong></div>
...
/* ===== ForgeLab v1.18.9 — Brouillon structuré : Programme > Trame > Blocs > Génération ===== */

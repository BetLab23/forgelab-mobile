...
function importPcProgrammingV165(){
  ...
  const marker=`forgelabMobileProgrammingImport:v1112:${currentUser.id}`;
...
async function repairSeriesFromCarnet(cloudRows){
  // v1.11.2 : fonction conservée uniquement pour compatibilité interne.
...
function jSeriesBaselineForDraft(id){
  ...
  // Compatibilité immédiate avec les séances déjà validées avant v1.11.2 :
...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1112.png" alt="" aria-hidden="true">
      <div class="auth-brand">FORGELAB</div>
      <div class="auth-sub">ATLAS · ESPACE PERSONNEL</div>
      <div class="auth-version"><span>v1.11.2</span></div>
    </div>
...
function drawTracking(){
  ...
  `<section class="tracking-hero">
    <div class="tracking-hero-copy"><small>SUIVI HEBDOMADAIRE</small><p>Vue d’ensemble de ta charge hebdomadaire.</p></div>
...
function seriesCleanupKey(){
  return `forgelabCleanup_S3_from_2026_09_15_v1112:${currentUser?.id||'guest'}`;
}
...
async function cleanupPhantomSeriesFromSep15(){
  ...
  console.info(`ForgeLab v1.11.2 : nettoyage S3 effectué (${before.length-after.length} brouillon(s) supprimé(s)).`);
}

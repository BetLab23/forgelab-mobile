...
async function repairSeriesFromCarnet(cloudRows){
  // v1.21.2 : fonction conservée uniquement pour compatibilité interne.
...
function jSeriesBaselineForDraft(id){
  ...
  // Compatibilité immédiate avec les séances déjà validées avant v1.21.2 :
...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1212.png" alt="" aria-hidden="true">
      <div class="auth-brand">FORGELAB</div>
      <div class="auth-sub">ATLAS · ESPACE PERSONNEL</div>
      <div class="auth-version"><span>v1.21.2</span></div>
    </div>
...
async function generateProgramImage(){
 ...
 const assets={};for(const f of ['auth-laurel-realistic-v1212.png','footer-mountains-man-v1212.png']){try{assets[f]=await load(f)}catch(_){}}
 ...
 const laurel=assets['auth-laurel-realistic-v1212.png'];
 ...
 const footer=assets['footer-mountains-man-v1212.png'];if(footer){g.save();g.globalAlpha=.66;const fw=W,fh=150,sy=Math.max(0,footer.height-footer.width*(fh/fw));g.drawImage(footer,0,sy,footer.width,footer.height-sy,0,H-footerH,fw,footerH);g.restore()}
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1212.png" alt=""><strong>ForgeLab</strong></div>
...
// v1.21.2 : aucun nettoyage automatique de Séries.
...
/* ===== ForgeLab v1.21.2 — Brouillon structuré : Programme > Trame > Blocs > Génération ===== */
...
function activateFullProgrammingDraft(){
  ...
  const now=new Date(),pad=n=>String(n).padStart(2,'0'),today=`${now.getFullYear()}-${pad(now.getMonth()+1)}-${pad(now.getDate())}`;
  const proposed=fp.startDate&&/^\d{4}-\d{2}-\d{2}$/.test(fp.startDate)?fp.startDate:today;
  const startDate=(prompt('Date de début du programme (S1) :',proposed)||'').trim();
  if(!/^\d{4}-\d{2}-\d{2}$/.test(startDate)){if(startDate)alert('Date invalide. Utilise le format AAAA-MM-JJ.');return}
  if(!confirm(`Rendre « ${fp.name||'ce brouillon'} » actif à partir du ${startDate.split('-').reverse().join('/')} ?\n\nLe programme actif actuel sera remplacé. Ton brouillon et tes paramètres existants sont conservés.`))return;
  fp.startDate=startDate;saveFullProgram(fp);
...
/* ===== ForgeLab v1.21.2 — Cardio progressif par bloc / semaine ===== */
...
function flCardioProgressivePanel(){
  const tpl=flCardioTemplate(),weeks=flCardioWeeks(),range=flSelectedBlockWeeks(),week=flCardioWeekSelected(),plan=Array.isArray(weeks[week])?weeks[week]:[];
  const weekBtns=Array.from({length:range.to-range.from+1},(_,i)=>range.from+i).map(w=>`<button type="button" class="${w===week?'active':''}" data-fl-cardio-week="${w}">S${w}</button>`).join('');
  ...
  const sessions=plan.map(s=>`<div class="fl-cardio-week-session" data-fl-cardio-session="${esc(s.id)}"><select data-flcw-day><option value="">Jour libre</option>${DAYS.map(d=>`<option ${d===s.day?'selected':''}>${d}</option>`).join('')}</select><select data-flcw-quality>${CARDIO_QUALITIES.map(x=>`<option ${x===s.quality?'selected':''}>${esc(x)}</option>`).join('')}</select><select data-flcw-engine>${CARDIO_ENGINES.map(x=>`<option ${x===s.engine?'selected':''}>${esc(x)}</option>`).join('')}</select><select data-flcw-format>${CARDIO_FORMATS.map(x=>`<option ${x===s.format?'selected':''}>${esc(x)}</option>`).join('')}</select><label><input data-flcw-duration type="number" min="0" max="300" value="${+s.duration||0}"><span>min</span></label><input data-flcw-target type="text" maxlength="50" placeholder="Objectif : 140–150 bpm…" value="${esc(s.target||'')}"><button type="button" class="fl-cardio-x" data-flcw-remove>×</button></div>`).join('');
  return `<section class="fl-cardio-progressive"><div class="fl-cardio-title"><div><small>♥ CARDIO · ${esc(selectedFullBlock()?.name||'BLOC')}</small><strong>Trame du bloc</strong><span>Définis seulement la fréquence et le type. Le détail se programme semaine par semaine.</span></div><button type="button" data-flct-add>＋</button></div><div class="fl-cardio-template-list">${rows}</div><div class="fl-cardio-week-head"><div><small>PROGRAMMATION HEBDOMADAIRE</small><strong>Décide au fil de ta progression</strong></div><div class="fl-cardio-week-tabs">${weekBtns}</div></div><div class="fl-cardio-week-actions"><button type="button" data-flcw-build>${plan.length?'Réinitialiser depuis la trame':'Programmer cette semaine'}</button>${plan.length?'<button type="button" class="secondary" data-flcw-add>＋ Ajouter une séance</button>':''}</div>${plan.length?`<div class="fl-cardio-week-list">${sessions}</div>`:`<div class="fl-cardio-week-empty"><b>S${week} non programmée</b><span>Tu pourras la définir quand tu auras le recul des semaines précédentes.</span></div>`}</section>`;
}
function flWireCardioProgressive(v){
  v.querySelectorAll('[data-fl-cardio-week]').forEach(b=>b.onclick=()=>flSetCardioWeekSelected(+b.dataset.flCardioWeek));
  ...
  const week=flCardioWeekSelected();v.querySelector('[data-flcw-build]')?.addEventListener('click',()=>{const x=flCardioWeeks();if(Array.isArray(x[week])&&x[week].length&&!confirm(`Réinitialiser le cardio de S${week} depuis la trame du bloc ?`))return;x[week]=flCardioPlanFromTemplate();flSaveCardioWeeks(x);drawProgramming()});
  v.querySelector('[data-flcw-add]')?.addEventListener('click',()=>{const x=flCardioWeeks();x[week]=Array.isArray(x[week])?x[week]:[];x[week].push({id:cardioModalityId(),day:'',quality:CARDIO_QUALITIES[0],engine:'Run',format:'Continu',duration:40,target:''});flSaveCardioWeeks(x);drawProgramming()});
  v.querySelectorAll('[data-fl-cardio-session]').forEach(row=>{const id=row.dataset.flCardioSession;const save=()=>{const x=flCardioWeeks(),s=(x[week]||[]).find(a=>a.id===id);if(!s)return;s.day=row.querySelector('[data-flcw-day]').value;s.quality=row.querySelector('[data-flcw-quality]').value;s.engine=row.querySelector('[data-flcw-engine]').value;s.format=row.querySelector('[data-flcw-format]').value;s.duration=Math.max(0,+row.querySelector('[data-flcw-duration]').value||0);s.target=row.querySelector('[data-flcw-target]').value.trim();flSaveCardioWeeks(x)};row.querySelectorAll('select,input').forEach(el=>el.addEventListener('change',save));row.querySelector('[data-flcw-remove]').onclick=()=>{const x=flCardioWeeks();x[week]=(x[week]||[]).filter(a=>a.id!==id);flSaveCardioWeeks(x);drawProgramming()}})
}
...
/* ===== ForgeLab v1.21.2 — Focus bloc + notes libres ===== */
...
drawProgramming=function(){
  ...
  if(programmingScreen==='chooser'||programmingMode()!=='draft'||!programBlockOpen)return;
  const v=document.querySelector('#view');if(!v)return;
  ...
  const actions=document.createElement('section');actions.className='fl-block-focus-actions';actions.innerHTML=`<button type="button" class="gold" data-fl-block-generate>Générer l’image de ce bloc</button>`;

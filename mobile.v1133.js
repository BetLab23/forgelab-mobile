...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1133.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 let ref=null,laurel=null;try{ref=await load('template-reference-v1133.png')}catch(_){ }try{laurel=await load('auth-laurel-realistic-v1133.png')}catch(_){ }
 ...
 box(0,0,W,94,'rgba(2,7,11,.995)');if(laurel)g.drawImage(laurel,381,8,78,78);txt('FORGELAB',470,37,34,white,900);txt('TRAIN  TRACK  PROGRESS',512,72,12,white,500,'center');line(0,94,W,94,blue,2);
 ...
 // Footer v1.13.3: conserver uniquement le décor montagnes + homme de dos et DAILY WINS.
 // Le personnage et les montagnes restent ceux du template validé ; toutes les anciennes citations sont masquées.
 if(ref){
   // Réaffiche toute la bande montagne du template d'origine.
   g.drawImage(ref,0,1224,1024,214,0,1224,1024,214);
 }else{
   box(0,1224,1024,214,'rgba(2,7,11,.98)');
 }
 // Masque les deux blocs de citation du template sans recouvrir l'homme à gauche.
 const grad=g.createLinearGradient(300,1224,1024,1224);
 grad.addColorStop(0,'rgba(3,8,12,.78)');grad.addColorStop(.18,'rgba(3,8,12,.94)');grad.addColorStop(1,'rgba(3,8,12,.96)');
 g.fillStyle=grad;g.fillRect(300,1224,724,214);
 txt('DAILY WINS.',715,1332,34,white,700,'center','Georgia');
 line(600,1365,830,1365,blue,3);
 // Pas de citation, pas de second logo ni de texte sous la scène finale.
 box(0,1438,1024,98,'rgba(2,7,11,.995)');
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1133.png" alt=""><strong>ForgeLab</strong></div>

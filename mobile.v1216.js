...
async function repairSeriesFromCarnet(cloudRows){
  // v1.21.6 : fonction conservée uniquement pour compatibilité interne.
...
function jSeriesBaselineForDraft(id){
  ...
  // Compatibilité immédiate avec les séances déjà validées avant v1.21.6 :
...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1216.png" alt="" aria-hidden="true">
      <div class="auth-brand">FORGELAB</div>
      <div class="auth-sub">ATLAS · ESPACE PERSONNEL</div>
      <div class="auth-version"><span>v1.21.6</span></div>
    </div>
...
async function generateProgramImage(){
 ...
 const assets={};for(const f of ['auth-laurel-realistic-v1216.png','footer-mountains-man-v1216.png']){try{assets[f]=await load(f)}catch(_){}}
 ...
 const laurel=assets['auth-laurel-realistic-v1216.png'];
 ...
 const footer=assets['footer-mountains-man-v1216.png'];if(footer){g.save();g.globalAlpha=.66;const fw=W,fh=150,sy=Math.max(0,footer.height-footer.width*(fh/fw));g.drawImage(footer,0,sy,footer.width,footer.height-sy,0,H-footerH,fw,footerH);g.restore()}
...
function flBlockPeriod(block,fp=flActiveProgram()){
  if(!fp?.startDate||!/^\d{4}-\d{2}-\d{2}$/.test(fp.startDate)||!block)return null;
  const [y,m,d]=fp.startDate.split('-').map(Number),from=Math.max(1,+block.from||1),to=Math.max(from,+block.to||from);
  const start=new Date(y,m-1,d,12);start.setDate(start.getDate()+(from-1)*7);
...
function exportBlockPdf(blockId){
  ...
  const period=flBlockPeriod(block,fp);if(!period){alert('La date de début du programme est nécessaire pour exporter le bilan du bloc.');return}
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1216.png" alt=""><strong>ForgeLab</strong></div>
...
// v1.21.6 : aucun nettoyage automatique de Séries.
...
/* ===== ForgeLab v1.21.6 — Brouillon structuré : Programme > Trame > Blocs > Génération ===== */
...
function activateFullProgrammingDraft(){
  ...
  // v1.21.6 : conserver aussi les notes et la trame cardio propres à chaque bloc lors de l'activation.
  fp.blocks.forEach(b=>{
...
drawProgramming=function(){
  ...
  const mode=programmingMode();if(mode!=='draft'){
    ...
    // v1.21.6 : le programme actif reprend la même architecture par blocs que le brouillon.
...
/* ===== ForgeLab v1.21.6 — Cardio du bloc : trame uniquement, séance dans Séries ===== */
...
/* ===== ForgeLab v1.21.6 — Focus bloc + notes libres ===== */
...
drawProgramming=function(){
  ...
  const actions=document.createElement('section');actions.className='fl-block-focus-actions';actions.innerHTML=`<button type="button" class="gold" data-fl-block-generate>Générer l’image de ce bloc</button>${programmingMode()==='active'?'<button type="button" class="secondary" data-fl-block-pdf>Exporter le bilan PDF</button>':''}`;

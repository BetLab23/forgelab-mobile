...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1206.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 const assets={};for(const f of ['auth-laurel-realistic-v1206.png','footer-mountains-man-v1206.png']){try{assets[f]=await load(f)}catch(_){}}
 ...
 const laurel=assets['auth-laurel-realistic-v1206.png'];if(laurel)g.drawImage(laurel,W/2-46,18,92,80);
 ...
 g.strokeStyle='rgba(228,187,104,.65)';g.lineWidth=1;g.beginPath();g.moveTo(72,290);g.lineTo(W-72,290);g.stroke();g.fillStyle=gold2;g.beginPath();g.moveTo(W/2,284);g.lineTo(W/2+7,290);g.lineTo(W/2,296);g.lineTo(W/2-7,290);g.closePath();g.fill();
 ...
 activeDays.forEach((day,i)=>{
   ...
   const rr=(xx,yy,ww,hh,r)=>{g.beginPath();g.roundRect(xx,yy,ww,hh,r)};g.save();g.shadowColor='rgba(0,0,0,.65)';g.shadowBlur=18;g.shadowOffsetY=7;rr(x,y,cardW,cardH,11);g.fillStyle='rgba(6,8,9,.965)';g.fill();g.restore();rr(x+.8,y+.8,cardW-1.6,cardH-1.6,10);g.strokeStyle='rgba(228,187,104,.78)';g.lineWidth=1.5;g.stroke();
 ...
 const footer=assets['footer-mountains-man-v1206.png'];if(footer){g.save();g.globalAlpha=.66;const fw=W,fh=150,sy=Math.max(0,footer.height-footer.width*(fh/fw));g.drawImage(footer,0,sy,footer.width,footer.height-sy,0,H-footerH,fw,footerH);g.restore()}
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1206.png" alt=""><strong>ForgeLab</strong></div>
...
/* ===== ForgeLab v1.20.6 — Brouillon structuré : Programme > Trame > Blocs > Génération ===== */

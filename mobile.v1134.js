...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1134.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 let ref=null,laurel=null,footerScene=null;try{ref=await load('template-reference-v1134.png')}catch(_){ }try{laurel=await load('auth-laurel-realistic-v1134.png')}catch(_){ }try{footerScene=await load('footer-mountains-man-v1134.png')}catch(_){ }
 ...
 // Footer v1.13.4 : uniquement la scène montagne + homme sur le rocher, sans aucune inscription.
 ...
 if(footerScene){
   ...
   // Prend la partie basse de la scène pour obtenir une bande panoramique : homme à gauche + chaîne sur toute la largeur.
   const cropH=Math.min(fh,Math.round(fw*0.34));
   const sy=Math.max(0,fh-cropH);
   g.drawImage(footerScene,0,sy,fw,cropH,0,1224,1024,312);
 }else if(ref){
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1134.png" alt=""><strong>ForgeLab</strong></div>

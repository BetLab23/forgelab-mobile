...
function importPcProgrammingV165(){
  ...
  const marker=`forgelabMobileProgrammingImport:v1103:${currentUser.id}`;
...
function jSeriesBaselineForDraft(id){
  ...
  // Compatibilité immédiate avec les séances déjà validées avant v1.10.3 :
...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1103.png" alt="" aria-hidden="true">
      <div class="auth-brand">FORGELAB</div>
      <div class="auth-sub">ATLAS · ESPACE PERSONNEL</div>
      <div class="auth-version"><span>v1.10.3</span></div>
    </div>
...
function drawTracking(){
  ...
  activePriorities().map(p=>{
    ...
    return`<article class="card ${p.priority.toLowerCase()} tracking-card muscle-order-card" data-muscle-order="${esc(p.muscle_key)}">
      ...
      <div class="trackline">${
        t<mi
          ? `<span>${pc}% du minimum</span><strong>${t} / ${mi}</strong>`
          : t>ma
            ? `<span>Maximum dépassé</span><strong>${t} séries · max ${ma}</strong>`
            : `<span>Dans la cible</span><strong>${t} séries · cible ${mi}–${ma}</strong>`
      }</div>
    </article>`
  }).join('');
  wireMuscleReorder(v);

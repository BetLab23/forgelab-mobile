...
function showAuth(message=''){
  ...
  box.innerHTML=`<main class="auth-screen"><section class="auth-shell">
    <div class="auth-identity">
      <img class="auth-laurel" src="auth-laurel-realistic-v1135.png" alt="" aria-hidden="true">
...
async function generateProgramImage(){
 ...
 let ref=null,laurel=null,footerScene=null;try{ref=await load('template-reference-v1135.png')}catch(_){ }try{laurel=await load('auth-laurel-realistic-v1135.png')}catch(_){ }try{footerScene=await load('footer-mountains-man-v1135.png')}catch(_){ }
 ...
 // Footer v1.13.5 : uniquement la scène montagne + homme sur le rocher, sans aucune inscription.
 // L'image est une ressource locale validée et intégrée au projet ; aucune génération n'est faite à l'exécution.
 if(footerScene){
   const fw=footerScene.naturalWidth||footerScene.width,fh=footerScene.naturalHeight||footerScene.height;
   // Fond panoramique : chaîne de montagnes sur toute la largeur.
   // On utilise uniquement la partie basse comme décor, puis on replace séparément
   // le personnage + son rocher afin qu'ils restent entièrement visibles.
   const bandH=Math.min(fh,Math.round(fw*0.34));
   const bandY=Math.max(0,fh-bandH);
   g.drawImage(footerScene,0,bandY,fw,bandH,0,1224,1024,312);

   // Personnage + rocher : crop vertical plus haut et plus large, affiché sans tronquer la tête.
   // Le ratio source est conservé (aucun étirement) et le bas est aligné sur le footer.
   const sx=0, sy=Math.max(0,Math.round(fh*0.705));
   const sw=Math.min(fw,Math.round(fw*0.36)), sh=fh-sy;
   const targetH=312, targetW=Math.round(sw*(targetH/sh));
   g.drawImage(footerScene,sx,sy,sw,sh,0,1224,targetW,targetH);
 }else if(ref){
...
function trackingMuscuSummary(){
  ...
  return `<section class="track-muscu-recap story-recap ${complete?'week-complete':''}">
    <div class="story-forgelab-brand"><img src="auth-laurel-realistic-v1135.png" alt=""><strong>ForgeLab</strong></div>
